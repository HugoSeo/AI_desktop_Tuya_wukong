...
/* Forward decl: driver init is defined after __tp_task but now invoked from it,
 * so the ~1s blocking startup runs in the tp thread instead of the caller. */
static OPERATE_RET tuya_tp_driver_init(ty_tp_device_cfg_t *device);

/* Current touch-level snapshot, updated on every press/move/release event in
 * tal_tp_write() (after the mirror transform) and held between events. Lets a
 * poller (e.g. the LVGL indev) read the *current* state via tal_tp_read_level()
 * instead of inferring "released" from an empty event queue — which wrongly
 * dropped a held finger to released whenever no new event was queued. Guarded
 * by g_tp_manage.mutex, same as the event queue. */
static ty_tp_point_info_t s_cur_level = {0};
...
static OPERATE_RET tal_tp_write(uint16_t x, uint16_t y, uint16_t state)
{
    ...
    do
    {
        ...
        /* Refresh the current-level snapshot regardless of queue fullness, so a
         * poller always sees the latest real state even if events are dropped. */
        s_cur_level.m_x     = point->m_x;
        s_cur_level.m_y     = point->m_y;
        s_cur_level.m_state = point->m_state;
...
static void __tp_task(void *args)
{
    ...
    /* Driver init (gpio/i2c + ~1s rst settle) runs here, in the tp thread, so
     * tal_tp_open() returns immediately and no longer blocks board init. The
     * sem is already created in tal_tp_open() before this thread starts, so an
     * ISR firing during init is recorded and consumed once the loop runs. On
     * failure touch simply won't work — log only, matching the async design. */
    if (tuya_tp_driver_init(device) != OPRT_OK)
    {
        PR_ERR("%s, tp driver init failed, touch disabled.\r\n", __func__);
    }
    ...
    while (1)
    {
        ...
        if (device->read_pont_info)
        {
            ...
            {
                tp_point_arr[i].tp_timestamp = tal_system_get_millisecond();
...
TY_TP_HANDLE tal_tp_open(ty_tp_device_cfg_t *device, ty_tp_usr_cfg_t *cfg)
{
    ...
    /* tuya_tp_driver_init() is deferred into __tp_task so this call returns
     * immediately instead of blocking the caller ~1s. */
...
OPERATE_RET tal_tp_read(ty_tp_point_info_t *point)
{
    ...
    do
    {
        ...
        {
            /* No queued transition: report the CURRENT level (held between
             * events) instead of an error, so a poller does not drop a held
             * finger to "released" just because no new event is queued. Real
             * press/move/release transitions still come through the queue below,
             * so fast taps between slow polls are preserved. */
            point->m_x             = s_cur_level.m_x;
            point->m_y             = s_cur_level.m_y;
            point->m_state         = s_cur_level.m_state;
            point->m_need_continue = 0;
            tal_mutex_unlock(g_tp_manage.mutex);
            break;   /* ret stays OPRT_OK */
        }
