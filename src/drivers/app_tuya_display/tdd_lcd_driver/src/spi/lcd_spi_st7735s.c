...
static const DISPLAY_INIT_SEQ_T  st7735s_init_seq[] = {
    ...
    /* MADCTL: 0xC8 | ML(bit4). ML 翻转面板垂直刷新方向（不改寻址映射），使
     * GRAM 写入与栅极扫描反向——两者行速几乎相同（~43 vs ~52us/行），同向时
     * 撕裂交叉被拉成宽过渡带，反向则一闪而过。 */
    {.type = TY_INIT_REG,   .reg = {.r = 0x36, .len = 1,  .v = {0xD8}}},            // Memory access control
...
const ty_display_device_s  lcd_spi_st7735s_device = {
    ...
    .spi = {
        .width = 128,
        .height = 128,
        /* 128x128 visible window sits at GRAM offset (0, 0x20) on this panel
         * (old framework used tkl_lvgl_display_offset_set(0, 0x20)); applied in
         * CASET/RASET by tal_display_spi via x_offset/y_offset. */
        .x_offset = 0,
        .y_offset = 0x20,
