...
static OPERATE_RET display_spi_pixel_send(ty_display_spi_device_t *device, ty_frame_buffer_t *frame_buff)
{
    ...
    /* Panel GRAM window offset (e.g. st7789p3 320x172 needs y_offset 0x22).
     * 0 for full-GRAM panels, so this is a no-op there. */
    UINT16_T x0 = frame_buff->x_start + device->x_offset;
    UINT16_T x1 = frame_buff->x_start + device->x_offset + frame_buff->width - 1;
    UINT16_T y0 = frame_buff->y_start + device->y_offset;
    UINT16_T y1 = frame_buff->y_start + device->y_offset + frame_buff->height - 1;
