...
typedef struct {
    ...
    /* Panel window offset into controller GRAM (CASET/RASET origin). Needed for
     * panels whose visible area is not at (0,0) of the controller RAM, e.g.
     * st7789p3 320x172 (y_offset 0x22). Defaults to 0 — full-GRAM panels
     * (st7789v2/st7789) leave these unset. Replaces the old framework's
     * tkl_lvgl_display_offset_set(). */
    UINT16_T x_offset;
    UINT16_T y_offset;
