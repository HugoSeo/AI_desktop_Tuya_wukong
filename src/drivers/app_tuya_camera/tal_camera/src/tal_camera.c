...
/* ------------------------------------------------------------------ */
/*  Relative size of a mode's yuv pingpong buffer                      */
/*  (tkl_dvp __dvp_yuv_buf_module_init: H264 = width*32*2,             */
/*   JPEG = width*16*2, pure YUV422 = none). Only used to decide       */
/*  whether a rollback is cheaper than the attempt that just failed.   */
/* ------------------------------------------------------------------ */
static UINT8_T __mode_buf_cost(TUYA_CAMERA_OUTPUT_MODE mode)
{
    switch (mode) {
    case TUYA_CAMERA_OUTPUT_H264:
    case TUYA_CAMERA_OUTPUT_H264_YUV422_BOTH:
    ...
    case TUYA_CAMERA_OUTPUT_JPEG_YUV422_BOTH:
        return 1;
    default:                /* pure YUV422 needs no pingpong buffer */
        return 0;
    }
}

/* ------------------------------------------------------------------ */
/*  DVP bring-up — wire the callbacks, mark the streams, init the drv  */
/* ------------------------------------------------------------------ */
static TUYA_DVP_DEVICE_T *__dvp_bringup(tal_camera_ctx_t       *ctx,
                                        TUYA_DVP_USR_CFG_T     *dvp_cfg,
                                        TUYA_CAMERA_OUTPUT_MODE mode)
{
...
TAL_CAMERA_HANDLE_T tal_camera_init(TAL_CAMERA_CFG_T *cfg)
{
    ...
    if (cfg->type == TAL_CAMERA_TYPE_DVP) {
        ...
        TUYA_DVP_DEVICE_T *dev = __dvp_bringup(ctx, dvp_cfg,
                                               dvp_cfg->dvp_cfg.output_mode);
        if (!dev) {
                                    ...
                                    TAL_STREAM_TYPE_E   stream)
{
    ...
    /* Without this the NULL sails into tal_dvp_start() and comes back as a
     * bare OPRT_INVALID_PARM, which reads like a bad argument from the caller
     * instead of "the driver was never brought back up". */
    if (!ctx->drv) {
        TAL_PR_ERR("tal_camera_start_stream: drv is NULL "
                   "(camera not initialized, or a mode switch failed to rebuild it)");
        return OPRT_COM_ERROR;
    }
                                          ...
                                          TUYA_CAMERA_OUTPUT_MODE mode)
{
    ...
    TUYA_DVP_USR_CFG_T     *dvp_cfg  = (TUYA_DVP_USR_CFG_T *)cfg->cfg;
    TUYA_CAMERA_OUTPUT_MODE old_mode = dvp_cfg->dvp_cfg.output_mode;

    /* drv == NULL means an earlier switch tore the DVP down and could not
     * bring it back (typically a transient OOM on the yuv pingpong buffer).
     * Don't refuse — fall through to the bring-up below and rebuild, else the
     * camera stays dead until reboot. */
    if (ctx->drv) {
        tal_dvp_stop((TUYA_DVP_DEVICE_T *)ctx->drv);
        tal_dvp_deinit((TUYA_DVP_DEVICE_T *)ctx->drv);
        ctx->drv = NULL;
    } else {
        TAL_PR_WARN("switch_output_mode: drv is NULL, rebuilding DVP");
    }

    if (__dvp_bringup(ctx, dvp_cfg, mode)) {
        TAL_PR_DEBUG("switch_output_mode ok, mode=%d, valid_fmts=0x%x", mode, ctx->valid_fmts);
        return OPRT_OK;
    }

    /* Target mode won't come up. Roll back to the mode we were running so the
     * camera keeps working (wrong mode, but alive) instead of being left with
     * drv == NULL forever — but skip a rollback that is hungrier than the
     * attempt that just failed (H264 pingpong is 2x JPEG): it cannot succeed
     * and each try costs a full sensor power/i2c-detect cycle. */
    if (old_mode == mode) {
        TAL_PR_ERR("switch_output_mode: DVP reinit failed (mode=%d), nothing to roll back to",
                   mode);
    } else if (__mode_buf_cost(old_mode) > __mode_buf_cost(mode)) {
        TAL_PR_ERR("switch_output_mode: DVP reinit failed (mode=%d), skip rollback to %d "
                   "(needs more memory than the failed attempt)", mode, old_mode);
    } else {
        TAL_PR_ERR("switch_output_mode: DVP reinit failed (mode=%d), rollback to %d",
                   mode, old_mode);
        if (__dvp_bringup(ctx, dvp_cfg, old_mode)) {
            TAL_PR_WARN("switch_output_mode: rolled back to mode %d", old_mode);
            return OPRT_COM_ERROR;
        }
        TAL_PR_ERR("switch_output_mode: rollback failed");
    }

    /* No usable mode left. Keep drv == NULL but make the state explicit: no
     * stream is servable, and the next switch retries from the rebuild branch
     * above. */
    ctx->valid_fmts              = 0;
    dvp_cfg->dvp_cfg.output_mode = old_mode;
    TAL_PR_ERR("switch_output_mode: camera is down");
    return OPRT_COM_ERROR;
}
