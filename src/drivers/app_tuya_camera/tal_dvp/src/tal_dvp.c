...
#define DVP_FLOW_EXIT_TIMEOUT_MS		(500)
#define DVP_FLOW_EXIT_POST_RETRY		(5)
...
static void __base_flow_task(void *args)
{
	...
	/* wait external tkl_thread_release */
	while (1)
		tal_system_sleep(10);
}
...
static VOID_T __dvp_flow_task_release(DVP_MANAGE_T *dvp_manage, DVP_WORK_FLOW_T work_flow)
{
	BOOL_T is_base = (work_flow == BASE_FRAME_FLOW);
	TKL_THREAD_HANDLE task;
	QUEUE_HANDLE *queue;
	SEM_HANDLE *closed_sem;
	CONST CHAR_T *name;
	...
	task = is_base ? dvp_manage->base_flow_thread : dvp_manage->encoded_flow_thread;
	queue = is_base ? &dvp_manage->base_frame_queue : &dvp_manage->encoded_frame_queue;
	closed_sem = is_base ? &dvp_manage->base_flow_closed_sem : &dvp_manage->encoded_flow_closed_sem;
	name = is_base ? "base" : "encoded";

	if (task) {
		tal_semaphore_create_init(closed_sem, 0, 1);
		if (*queue) {
			for (UINT8_T i = 0; i < DVP_FLOW_EXIT_POST_RETRY; i++) {
				if (tal_queue_post(*queue, &msg, 0) == OPRT_OK) {
					exit_ready = true;
					break;
				}
				PR_ERR("post %s EXIT fail, retry %d/%d\r\n", name, i + 1, DVP_FLOW_EXIT_POST_RETRY);
				tal_system_sleep(1);
			}
		} else {
			PR_ERR("%s queue NULL, force delete\r\n", name);
		}

		if (exit_ready && *closed_sem) {
			if (tal_semaphore_wait(*closed_sem, DVP_FLOW_EXIT_TIMEOUT_MS)) {
				PR_ERR("wait %s EXIT timeout, force delete\r\n", name);
			}
		} else if (!exit_ready) {
			PR_ERR("%s EXIT not posted, force delete\r\n", name);
		}

		tal_system_sleep(2);
		tkl_thread_release(task);
	}

	if (*closed_sem) {
		tal_semaphore_release(*closed_sem);
		*closed_sem = NULL;
	}
	if (*queue) {
	...
	__dvp_node_deinit(work_flow, dvp_manage);
	if (is_base) {
...
static OPERATE_RET __dvp_work_flow_init(DVP_MANAGE_T *dvp_manage)
{
	...
	if (dvp_manage->base_flow_flag)
	{
		...
		ret |= tkl_thread_create_in_psram(&(dvp_manage->base_flow_thread), "dvp_base", 16384, THREAD_PRIO_1, __base_flow_task, (VOID_T *)dvp_manage);
	}

	if (dvp_manage->encoded_flow_flag)
	{
		ret |= __dvp_node_init(ENCODED_FRAME_FLOW, encoded_len, dvp_manage);
		ret |= tkl_thread_create_in_psram(&(dvp_manage->encoded_flow_thread), "dvp_encode", 16384, THREAD_PRIO_1, __encoded_flow_task, (VOID_T *)dvp_manage);
	}
...
static OPERATE_RET __dvp_work_flow_deinit(DVP_MANAGE_T *dvp_manage)
{
	...
	if (dvp_manage->base_flow_flag) {
		__dvp_flow_task_release(dvp_manage, BASE_FRAME_FLOW);
	}
	if (dvp_manage->encoded_flow_flag) {
		__dvp_flow_task_release(dvp_manage, ENCODED_FRAME_FLOW);
	}
