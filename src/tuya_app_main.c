...
#endif
#if defined(ENABLE_APP_KV_CACHE) && defined(ENABLE_KV_CACHE)
...
/* ui_svc_activate.c 只在 wukong 页面集构建时编译（local.mk 的 services glob 包在
 * CONFIG_UI_WUKONG_PAGES 内）；board-UI 板型（ENABLE_TUYA_UI=y、UI_WUKONG_PAGES=n）
 * 必须一并排除，否则链接 undefined reference。 */
#if defined(ENABLE_TUYA_UI) && (ENABLE_TUYA_UI == 1) && \
    defined(UI_WUKONG_PAGES) && (UI_WUKONG_PAGES == 1)
 ...
 * Parses the short URL out of the JSON payload, feeds it to the activation QR page
 * (ENABLE_TUYA_UI) and prints an ANSI QR code on Linux hosts.
 * @param[in] shorturl JSON string containing the short URL. NULL means the URL became
 * stale (direct-MQTT link dropped) and any rendered QR code should be cleared.
 */
STATIC VOID __qrcode_active_shorturl_cb(CONST CHAR_T *shorturl)
{
    ...
    if (NULL != shorturl) {
        TAL_PR_INFO("shorturl : %s", shorturl);
        item = ty_cJSON_Parse(shorturl);
        if (NULL != item) {
            /* The SDK emits the field as lowercase "shorturl" (svc_netcfg_qrcode.c);
             * older payloads used "shortUrl" — accept both. */
            ty_cJSON *url_obj = ty_cJSON_GetObjectItem(item, "shorturl");
            if (NULL == url_obj) {
                url_obj = ty_cJSON_GetObjectItem(item, "shortUrl");
            }
            if (NULL != url_obj && NULL != url_obj->valuestring) {
                url = url_obj->valuestring;
            }
        } else {
            url = shorturl;   /* not JSON: treat as a plain URL */
        }
    }

#if defined(ENABLE_TUYA_UI) && (ENABLE_TUYA_UI == 1) && \
    defined(UI_WUKONG_PAGES) && (UI_WUKONG_PAGES == 1)
    ui_svc_activate_notify_shorturl(url);
#endif

#if OPERATING_SYSTEM == SYSTEM_LINUX
    if (NULL != url) {
        __qrcode_printf((CHAR_T *)url);
    }
#endif
...
#if defined(ENABLE_WIFI_SERVICE) && (ENABLE_WIFI_SERVICE == 1)
#if defined(ENABLE_WIFI_ULTRA_LOWPOWER) && (ENABLE_WIFI_ULTRA_LOWPOWER == 1) \
        && defined(TY_AI_LOW_POWER_MODE) && (TY_AI_LOW_POWER_MODE == 2)
    /* Ultra low power (AP keep-alive): bring up ULP stack, lpmgr-managed (DTIM-10). */
    tuya_wifi_ulp_init();
    lpmgr_set_lps_dtim(TY_LP_DITM_10);
    lpmgr_register(TY_LP_APP_USED);
#else
    /* Light/normal: configure WiFi low-power (DTIM 3); LP disabled after config. */
...
#endif
#if (defined(ENABLE_QRCODE_ACTIVE) && (ENABLE_QRCODE_ACTIVE == 1))
    /* 兜底注册扫码激活服务：设备手动连过 WiFi（nc_tp 已非 UNCFG）但未激活就重启时，
     * SDK 不再注册 netcfg 服务组，短链流程会缺失。init 只做一次性 LINK_UP 订阅
     * （base_event 按 desc+cb 去重），已激活时其回调自行跳过，重复调用无副作用。 */
    tuya_svc_netcfg_qrcode_init(NULL);
#endif
...
#if defined(ENABLE_TUYA_UI) && (ENABLE_TUYA_UI == 1)
    /* 所有 UI 调用收口于 ui_app_init()：内部完成 display-bus init、调用已注册的 board UI
     * 或默认 wukong 路径。显示硬件 init 已在各 board 的 tuya_device_board_init() 中完成。 */
    ui_app_init();
#endif
    return rt;
#endif
...
#if defined(ENABLE_APP_KV_CACHE) && defined(ENABLE_KV_CACHE)
/**
 * @brief KV cache sync timeout callback.
 *
 * Called by the KV cache sync timer before each Flash sync. Returns the retry
 * delay in ms when audio is playing so the sync is deferred until playback
 * ends. Returns 0 to allow an immediate sync when idle.
 */
STATIC UINT_T __kv_cache_sync_timeout_cb(VOID *user_data)
{
    if (wukong_audio_player_is_playing()) {
        return KV_CACHE_SYNC_INTERVAL_MS;
    }
    return 0;
}
#endif
...
STATIC VOID_T user_main(VOID_T)
{
...
#if defined(ENABLE_APP_KV_CACHE) && defined(ENABLE_KV_CACHE)
    TUYA_CALL_ERR_LOG(ws_db_cache_enable(0));
    TUYA_CALL_ERR_LOG(ws_db_register_sync_timeout_cb(__kv_cache_sync_timeout_cb, NULL));
#endif
#endif
#endif
    ...
    /* Kick off the single app-wide external-storage mount; it runs on
     * WORKQ_SYSTEM and never blocks boot (see src/wukong/storage/).
     *
     * Must come after the IoT init above: WORKQ_SYSTEM is created by
     * tal_workq_init(), which tuya_iot_core_init() calls. Scheduling before
     * that makes tal_workq_get_handle() return NULL, the work is silently
     * never queued (only a bare "ret:-2" from tal_workq_service.c), the volume
     * never mounts and every storage consumer degrades for the whole boot. */
    TUYA_CALL_ERR_LOG(wukong_storage_init());
