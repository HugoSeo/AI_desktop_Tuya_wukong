...
STATIC OPERATE_RET wukong_ai_oneshot_task_cb(VOID *data, INT_T len)
{
    ...
    AI_CHAT_STATE_E state = s_ai_oneshot.state;
 ...
 */
/**
 * @brief One-shot image recognition in oneshot mode.
 *
 ...
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET wukong_ai_oneshot_picture(VOID *data, INT_T len)
{
    return wukong_ai_mode_picture_recognize(AI_CHAT_SUB_ONESHOT, &s_ai_oneshot.state, data, len);
}

/**
 * @brief Terminate the current turn in oneshot mode (e.g. when the camera opens).
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET wukong_ai_oneshot_interrupt(VOID *data, INT_T len)
{
    (VOID)data;
    (VOID)len;
    return wukong_ai_mode_interrupt_turn(AI_CHAT_SUB_ONESHOT, &s_ai_oneshot.state);
}
...
OPERATE_RET ai_oneshot_register(AI_CHAT_MODE_HANDLE_T **cb)
{
    ...
    s_ai_oneshot_cb.on_picture    = wukong_ai_oneshot_picture;
    s_ai_oneshot_cb.on_interrupt  = wukong_ai_oneshot_interrupt;
    *cb = &s_ai_oneshot_cb;
    return rt;
}
#endif
