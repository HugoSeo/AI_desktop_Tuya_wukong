...
STATIC OPERATE_RET wukong_ai_picture_deint_cb(VOID *data, INT_T len)
{
 ...
 STATIC OPERATE_RET wukong_ai_picture_task_cb(VOID *data, INT_T len)
 {
     ...
     AI_CHAT_STATE_E state = s_ai_picture.state;
 ...
 STATIC OPERATE_RET wukong_ai_picture_event_cb(VOID *data, INT_T len)
 {
     ...
     switch (event->type) 
     {
        ...
        case WUKONG_AI_EVENT_PLAY_CTL_PLAY:
         ...
         {
             /* The image is sent as soon as recording starts, while the user is
              * still holding the talk key.  This event only completes the image
              * stream; it must not end the voice turn or clear its preview.
              * RELEASE_KEY commits the complete image + voice request below. */
         }
         break;
