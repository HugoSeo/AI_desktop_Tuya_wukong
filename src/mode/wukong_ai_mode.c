...
/** Display names for AI_DEVICE_MODE_E (indexed by enum value); keep in sync
 *  with AI_DEVICE_MODE_MAX and the labels used in ui_control_center / ui_device_mode. */
STATIC CONST CHAR_T *s_device_mode_display[AI_DEVICE_MODE_MAX] = {
    "闲聊模式",  /* AI_DEVICE_MODE_CHAT */
    "翻译模式",  /* AI_DEVICE_MODE_TRANSLATE */
    "P2P模式",   /* AI_DEVICE_MODE_P2P */
    "录音模式",  /* AI_DEVICE_MODE_RECORD */
    "生图模式",  /* AI_DEVICE_MODE_PICTURE */
    "侦测模式",  /* AI_DEVICE_MODE_DETECTION */
};

STATIC CONST CHAR_T *s_chat_sub_display[AI_CHAT_SUB_MAX] = {
    "长按",  /* AI_CHAT_SUB_HOLD */
    "按键",  /* AI_CHAT_SUB_ONESHOT */
    "唤醒",  /* AI_CHAT_SUB_WAKEUP */
    "自由",  /* AI_CHAT_SUB_FREE */
};
...
/**
 * @brief Post a "已成功切换到XX模式" notification to the chat UI
 * @return none
 * @note No-op when ENABLE_TUYA_UI is not defined. The notification is delivered
 *       asynchronously via tuya_ai_display_msg, which copies the buffer
 *       internally, so passing a stack buffer is safe.
 */
STATIC VOID_T __post_mode_switched_notify(VOID_T)
{
#if defined(ENABLE_TUYA_UI)
    AI_DEVICE_MODE_E mode = s_mode_mgr.device_mode;
    CHAR_T buf[64];
    INT_T len;
    ...
    if (mode == AI_DEVICE_MODE_CHAT) {
        ...
        CONST CHAR_T *sub_name = (sub < AI_CHAT_SUB_MAX) ? s_chat_sub_display[sub] : "";
        len = snprintf(buf, sizeof(buf), "已成功切换到%s: %s",
                       s_device_mode_display[mode], sub_name);
    } else {
        len = snprintf(buf, sizeof(buf), "已成功切换到%s",
                       s_device_mode_display[mode]);
    }
    ...
    if ((UINT32_T)len >= sizeof(buf)) {
        len = (INT_T)sizeof(buf) - 1;
    }

    tuya_ai_display_msg((UINT8_T *)buf, len, TY_DISPLAY_TP_MODE_NOTIFY);
#endif
}
...
STATIC OPERATE_RET __mode_dispatch(AI_MODE_OP_E op, VOID *data, INT_T len)
{
    ...
    case AI_MODE_OP_PICTURE:
        MODE_DISPATCH_CALL(h, on_picture, data, len, "picture");

    case AI_MODE_OP_INTERRUPT:
        MODE_DISPATCH_CALL(h, on_interrupt, data, len, "interrupt");
...
/**
 * @brief Cleanly abort whatever turn is in flight on the shared CHAT session.
 *
 * The standard barge-in idiom (ADR-0005): stop cloud TTS output, stop the local
 * player, reset the audio frontend (drops the mic queue + resets VAD so no stale
 * frame races what comes next), and tell the cloud to break the current chat.
 * Does NOT touch the per-mode state machine — the caller decides the next state.
 */
STATIC VOID __barge_in_turn(VOID)
{
...
OPERATE_RET wukong_ai_mode_interrupt_turn(AI_CHAT_SUB_MODE_E sub, AI_CHAT_STATE_E *state)
{
    if (state == NULL) {
        return OPRT_INVALID_PARM;
    }

    /* Unconditionally terminate the current turn and settle back to IDLE. Unlike
     * picture recognition, no new input follows, so also close any input
     * transaction that may still be open (e.g. mid LISTEN/UPLOAD). */
    __barge_in_turn();
    wukong_ai_agent_input_stop();
    CHAT_SUB_STATE_CHANGE(sub, *state, AI_CHAT_IDLE);

    return OPRT_OK;
}

OPERATE_RET wukong_ai_mode_picture_recognize(AI_CHAT_SUB_MODE_E sub, AI_CHAT_STATE_E *state,
                                             VOID *jpeg, INT_T len)
{
    if (jpeg == NULL || len <= 0 || state == NULL) {
        return OPRT_INVALID_PARM;
    }

    /* Cleanly abort whatever turn is in flight before the image-recognition
     * turn (ADR-0005). */
    __barge_in_turn();

    /* This is an image-only turn: mark THINK (awaiting cloud), NOT LISTEN.
     * LISTEN makes the mode's task pump (re)arm the mic for a speech turn, whose
     * frames then mix into this transaction and the cloud returns ASR_EMPTY
     * instead of the image result. From THINK the cloud ASR/TTS events still
     * drive THINK -> SPEAK -> IDLE for the streamed reply. */
    CHAT_SUB_STATE_CHANGE(sub, *state, AI_CHAT_THINK);

    /* One image-only input transaction on the CHAT scode. Mute the mic at the
     * SDK audio ringbuf for the duration: free mode keeps listening even in
     * THINK, and any mode may have a mic frame in flight, so muting guarantees
     * no audio mixes into this transaction. Image/text upload on a separate
     * path and are unaffected by the mute (see ADR-0005). */
    tuya_ai_input_mute_audio(TRUE);
    wukong_ai_agent_set_scene(AI_AGENT_SCODE_CHAT);
    wukong_ai_agent_input_start(TRUE);
    wukong_ai_agent_send_image((UINT8_T *)jpeg, (UINT_T)len);
...
OPERATE_RET wukong_ai_device_mode_switch(AI_DEVICE_MODE_E mode)
{
    ...
    /* 去重:已处于目标设备模式则直接返回,避免无谓的 DEINIT/del_session/INIT。
     * (CHAT 子模式切换由 wukong_ai_chat_sub_mode_switch 负责,不走此路径) */
    if (mode == s_mode_mgr.device_mode) {
        TAL_PR_DEBUG("[ai_mode] already in device mode %d (%s), skip", mode, _device_mode_str[mode]);
        return OPRT_OK;
    }
    ...
    if (old_scode != NULL) {
        ...
        wukong_ai_agent_del_session(old_scode);
    }
    ...
    if (mode == AI_DEVICE_MODE_CHAT) {
        /* Re-entering CHAT activates chat_subs[chat_sub_mode]; sync it back to
         * the persisted trigger mode so cfg never lags the running sub-mode
         * (no-op when already equal). */
        tuya_ai_toy_trigger_mode_set(s_mode_mgr.chat_sub_mode);
    }
    wukong_ai_mode_dispatch(AI_MODE_OP_INIT, NULL, 0);

    __post_mode_switched_notify();
...
STATIC OPERATE_RET __default_audio_input(VOID *data, INT_T len)
{
    ...
    if (!wukong_ai_agent_is_ready()) {
        TAL_PR_DEBUG("ai agent is not ready, ignore audio input");
        return OPRT_OK;
    }

    TAL_PR_DEBUG("ai toy -> send wukong mic data: %d", len);

    TUYA_CALL_ERR_LOG(wukong_ai_agent_send_audio((UINT8_T *)data, (UINT_T)len));
...
OPERATE_RET wukong_ai_mode_init(VOID)
{
    ...
    if (stored_dev == AI_DEVICE_MODE_CHAT) {
        ...
        /* Seed chat_sub_mode from the stored trigger mode so a later switch
         * back to CHAT restores the user's choice instead of silently falling
         * back to the zero-value default (HOLD), which also desynced the
         * manager from the persisted cfg read by the UI. */
        if (stored_sub < AI_CHAT_SUB_MAX && s_mode_mgr.chat_subs[stored_sub].enabled) {
            s_mode_mgr.chat_sub_mode = stored_sub;
        }
        TAL_PR_DEBUG("[ai_mode] init device mode %d (%s), chat sub-mode %d (%s)",
                     stored_dev, _device_mode_str[stored_dev],
                     s_mode_mgr.chat_sub_mode, _chat_sub_str[s_mode_mgr.chat_sub_mode]);
        wukong_ai_mode_dispatch(AI_MODE_OP_INIT, NULL, 0);
        return OPRT_OK;
    }
