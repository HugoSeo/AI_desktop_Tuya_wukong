...
STATIC OPERATE_RET wukong_ai_hold_int_cb(VOID *data, INT_T len)
{
    ...
    // disable vad
    wukong_ai_agent_server_vad_ctrl(FALSE);
...
STATIC OPERATE_RET wukong_ai_hold_deint_cb(VOID *data, INT_T len)
{
    ...
    // enable vad
    wukong_ai_agent_server_vad_ctrl(TRUE);
    return rt;
}
...
{
    /* Snapshot the live state once: other threads can rewrite it while a
     * state callback below is still running. Dispatch and the s_ai_cur_state
     * bookkeeping must use the same value, otherwise the newer state is
     * marked handled without its callback ever running. */
    AI_CHAT_STATE_E state = s_ai_hold.state;
 ...
 */
/**
 * @brief One-shot image recognition in hold mode.
 *
 ...
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET wukong_ai_hold_picture(VOID *data, INT_T len)
{
    return wukong_ai_mode_picture_recognize(AI_CHAT_SUB_HOLD, &s_ai_hold.state, data, len);
}

/**
 * @brief Terminate the current turn in hold mode (e.g. when the camera opens).
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET wukong_ai_hold_interrupt(VOID *data, INT_T len)
{
    (VOID)data;
    (VOID)len;
    return wukong_ai_mode_interrupt_turn(AI_CHAT_SUB_HOLD, &s_ai_hold.state);
}
...
OPERATE_RET ai_hold_register(AI_CHAT_MODE_HANDLE_T **cb)
{
    ...
    s_ai_hold_cb.on_picture    = wukong_ai_hold_picture;
    s_ai_hold_cb.on_interrupt  = wukong_ai_hold_interrupt;
    *cb = &s_ai_hold_cb;
    return rt;
}
#endif
