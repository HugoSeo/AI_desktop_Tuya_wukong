...
#if defined(ENABLE_TUYA_UI) || defined(ENABLE_TUYA_DISPLAY)
...
typedef struct {
    ...
    OPERATE_RET (*on_picture)       (VOID *data, INT_T len);  /**< optional; one-shot image recognition (see ADR-0005) */
    OPERATE_RET (*on_interrupt)     (VOID *data, INT_T len);  /**< optional; terminate the current turn (e.g. on opening the camera) */
...
/**
 * @brief Barge-in the current turn and run a one-shot image-recognition
 *        input transaction on the shared CHAT session.
 *
 * Shared by every chat sub-mode's on_picture so the fix lives in one place and
 * each mode keeps its own state machine consistent (see docs/adr/0005). Performs
 * the standard barge-in (output_stop + audio_player_stop + audio_input_reset +
 * chat_break), marks the caller's state as a fresh LISTEN turn, then runs
 * input_start -> image -> text -> input_stop on the CHAT scode.
 *
 * @param[in]     sub   Active chat sub-mode (state bookkeeping + logging).
 * @param[in,out] state The mode's own chat state; reset to LISTEN for the new turn.
 * @param[in]     jpeg  JPEG image data (borrowed; deep-copied into the agent send path).
 * @param[in]     len   JPEG length in bytes.
...
/**
 * @brief Unconditionally terminate the current chat turn and settle to IDLE.
 *
 * Shared by every chat sub-mode's on_interrupt. Runs the standard barge-in
 * (output_stop + audio_player_stop + audio_input_reset + chat_break), closes any
 * open input transaction, and resets the caller's state to IDLE. Used e.g. when
 * the camera page opens so a dialogue in progress is cleanly stopped (ADR-0005).
 *
 * @param[in]     sub   Active chat sub-mode (state bookkeeping + logging).
 * @param[in,out] state The mode's own chat state; reset to IDLE.
 * @return OPRT_OK on success, OPRT_INVALID_PARM on bad args.
 */
