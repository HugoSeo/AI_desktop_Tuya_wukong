...
STATIC OPERATE_RET wukong_ai_free_task_cb(VOID *data, INT_T len)
{
    ...
    AI_CHAT_STATE_E state = s_ai_free.state;
...
STATIC OPERATE_RET wukong_ai_free_vad(VOID *data, INT_T len)
{
    ...
    if (WUKONG_AUDIO_VAD_START == vad_flag) 
    {
        ...
        wukong_ai_agent_input_start(FALSE);
        tuya_ai_toy_idle_timer_ctrl(FALSE);
    } 
    else 
    {
 ...
 */
/**
 * @brief One-shot image recognition in free mode.
 *
 ...
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET wukong_ai_free_picture(VOID *data, INT_T len)
{
    return wukong_ai_mode_picture_recognize(AI_CHAT_SUB_FREE, &s_ai_free.state, data, len);
}

/**
 * @brief Terminate the current turn in free mode (e.g. when the camera opens).
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET wukong_ai_free_interrupt(VOID *data, INT_T len)
{
    (VOID)data;
    (VOID)len;
    return wukong_ai_mode_interrupt_turn(AI_CHAT_SUB_FREE, &s_ai_free.state);
}
...
OPERATE_RET ai_free_register(AI_CHAT_MODE_HANDLE_T **cb)
{
    ...
    s_ai_free_cb.on_picture    = wukong_ai_free_picture;
    s_ai_free_cb.on_interrupt  = wukong_ai_free_interrupt;
    *cb = &s_ai_free_cb;
    return rt;
}
#endif
