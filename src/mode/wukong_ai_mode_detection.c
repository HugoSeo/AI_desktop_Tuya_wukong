 ...
 * --------------------------------------------------------------------------- */
STATIC VOID __on_yuv_frame(const VIDEO_FRAME_T *frame, VOID *ctx);
...
/**
 * @brief Push the mode's current chat state to the UI title-bar status label.
 *
 * The display layer derives the title-bar status from the text-stream
 * lifecycle (AI_CHAT_START -> speaking, AI_CHAT_STOP -> idle). Detection's
 * text stream ends well before TTS playback does, so without re-asserting the
 * mode's authoritative state the label would flip to "idle" mid-speech. This
 * keeps it in sync with the real state machine.
 */
STATIC VOID_T __md_ui_sync_chat_stat(VOID_T)
{
    AI_CHAT_STATE_E state = s_ai_detection.state;
...
#if defined(ENABLE_TUYA_UI) || defined(ENABLE_TUYA_DISPLAY)
    /* Playback has truly ended now: bring the title-bar status back to idle. */
    __md_ui_sync_chat_stat();
#endif
    tuya_camera_detection_start(__on_yuv_frame, NULL);
...
#endif
#if defined(ENABLE_TUYA_UI) || defined(ENABLE_TUYA_DISPLAY)
        /* AI_CHAT_STOP makes the display layer flip the title-bar status to
         * idle, but the text stream finishes before TTS playback. Re-assert the
         * real state (still SPEAK until PLAY_END) so it stays "speaking" until
...
/**
 * @brief YUV frame callback registered to the camera adaptation layer
 */
STATIC VOID __on_yuv_frame(const VIDEO_FRAME_T *frame, VOID *ctx)
{
    (VOID)ctx;
    if (!frame || !s_md_inited || s_md_y_buf == NULL) {
        return;
    }
    ...
    __extract_y_downsample(frame->data, frame->width, frame->height,
...
STATIC OPERATE_RET wukong_ai_detection_task_cb(VOID *data, INT_T len)
{
    ...
    /* Capture one JPEG frame synchronously from the MJPEG stream (independent
     * of any UI/MCP photo path). The adapter fans out to the SNAPSHOT
     * consumer without disrupting other subscribers. */
    BYTE_T *image_data = NULL;
    UINT_T image_size = 0;
    OPERATE_RET rt = tuya_camera_snapshot(&image_data, &image_size, 5000);
    ...
    wukong_ai_agent_set_scene(AI_AGENT_SCODE_DETECTION);
