...
STATIC TAL_AUDIO_INPUT_HANDLE s_mf_audio_input = NULL;
STATIC TAL_AUDIO_OUTPUT_HANDLE s_mf_audio_output = NULL;
...
STATIC OPERATE_RET __mf_audio_output_write(UINT8_T *data, UINT32_T len);
...
STATIC VOID_T __pt_record_thread_entry(VOID *arg)
{
    ...
    if (tuya_ring_buff_used_size_get(stream_ringbuf) > 0) {
        ...
        do {
            ...
            rt = __mf_audio_output_write((UINT8_T *)buf, (UINT32_T)read_size);
            if (rt != OPRT_OK) {
                TAL_PR_ERR("mf audio output write failed, ret=%d", rt);
                continue;
            }
        } while (1);
        tal_free(buf);
        ...
        mf_cmd_send(0xF0, (BYTE_T *)ret_buf, buf_len);
        tal_free(ret_buf);
    } else {
...
STATIC OPERATE_RET __pt_record_test(USHORT_T cmd, UCHAR_T *data, UINT_T len, UCHAR_T **ret_data, USHORT_T *ret_len)
{
    ...
    TUYA_CHECK_NULL_GOTO(json_sec, EXIT);
...
STATIC VOID_T __mf_thread_entry(VOID *arg)
{
    ...
    for (UCHAR_T i = 0; i < CNTSOF(key_gpios); i++) {
        if (key_gpios[i] >= TUYA_GPIO_NUM_MAX) {
            TAL_PR_ERR("key gpio %d is disabled", key_gpios[i]);
            continue;     // 跳过 disabled (=64) 的 pin，避免越界
        }
...
STATIC VOID_T _audio_frame_put(TAL_AUDIO_FRAME_T *frame, VOID_T *args)
{
    (void)args;
    ...
    // TAL_PR_DEBUG("audio frame put, size:%d, record_start:%d, stream_ringbuf=%p", frame->len, record_start, stream_ringbuf);
    if (!stream_ringbuf) {
        TAL_PR_ERR("stream_ringbuf is NULL");
        return;
    }

    tuya_ring_buff_write(stream_ringbuf, frame->buf, frame->len);
}
...
STATIC OPERATE_RET __mf_audio_init(VOID)
{
    ...
    config.dev.ai_adc_conf.chan = TUYA_AUDIO_ADC_CHANNEL_L;
    config.sample_bits = TUYA_AUDIO_SAMPLE_BITS_16;
    config.sample_rate = AUDIO_SAMPLE_RATE;
    config.frame_time_ms = 20;
    config.audio_input_cb = _audio_frame_put;

    TAL_PR_NOTICE("tal_audio_input_init...");
    s_mf_audio_input = tal_audio_input_init(&config);
    if (s_mf_audio_input == NULL) {
        rt = OPRT_COM_ERROR;
        goto err;
    }

    TAL_PR_NOTICE("tal_audio_input_start...");
    TUYA_CALL_ERR_GOTO(tal_audio_input_start(s_mf_audio_input), err);
    tal_audio_input_set_volume(s_mf_audio_input, 100);

    TAL_AUDIO_OUTPUT_CFG_T output_config = {0};
    output_config.type = TAL_AUDIO_OUTPUT_DAC;
    output_config.dev.dac_config.port = TUYA_AUDIO_DAC_PORT_0;
    output_config.dev.dac_config.spk_num = 1;
    output_config.dev.dac_config.pa_gpio = TUYA_AI_TOY_SPK_EN_PIN;
    output_config.dev.dac_config.pa_active_level = TUYA_GPIO_LEVEL_HIGH;
    output_config.sample_bits = TUYA_AUDIO_SAMPLE_BITS_16;
    output_config.sample_rate = SPK_SAMPLE_RATE;
    output_config.frame_time_ms = 20;

    TAL_PR_NOTICE("tal_audio_output_init...");
    s_mf_audio_output = tal_audio_output_init(&output_config);
    if (s_mf_audio_output == NULL) {
        rt = OPRT_COM_ERROR;
        goto err;
    }

    TAL_PR_NOTICE("tal_audio_output_start...");
    TUYA_CALL_ERR_GOTO(tal_audio_output_start(s_mf_audio_output), err);
...
{
    if (s_mf_audio_input != NULL) {
        tal_audio_input_stop(s_mf_audio_input);
        tal_audio_input_deinit(s_mf_audio_input);
        s_mf_audio_input = NULL;
    }
    if (s_mf_audio_output != NULL) {
        tal_audio_output_stop(s_mf_audio_output);
        tal_audio_output_deinit(s_mf_audio_output);
        s_mf_audio_output = NULL;
    }
    return OPRT_OK;
}
...
STATIC OPERATE_RET __mf_audio_output_write(UINT8_T *data, UINT32_T len)
{
    return tal_audio_output_write(s_mf_audio_output, data, len);
}
