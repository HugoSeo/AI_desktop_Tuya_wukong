 ...
 *
 * Decodes the JPEG at the smallest size the codec can produce that still covers
 * the requested output, then scales that down to the exact target. Decoding at
 * a reduced size keeps the intermediate buffer proportional to the result rather
 * than to the source, which matters on a fragmented PSRAM heap where a
 * full-resolution buffer may simply not fit. The intermediate buffer is freed
 * before returning, unless the reduced decode already lands on the target size,
 * in which case it becomes the output and no resample pass runs at all.
 *
                                ...
                                __image_scale_fn_t scale)
{
    ...
    /* Step 3: decode JPEG to a reduced-size raw buffer */
    uint16_t dec_w, dec_h;
    rt = tal_image_jpeg_calc_scaled_size(info.width, info.height,
                                         out_w, out_h, &dec_w, &dec_h);
    if (rt != OPRT_OK) {
        return rt;
    }

    uint32_t decode_size = (uint32_t)dec_w * dec_h * bpp;
    ...
    if (dec_w == out_w && dec_h == out_h) {
        out->buf    = decode_buf;
        out->size   = (uint32_t)out_w * out_h * bpp;
        out->width  = out_w;
        out->height = out_h;
        return OPRT_OK;
    }
 ...
 *
 * Decodes at the smallest codec-supported size that still covers the requested
 * dimensions, then scales down to them. The temporary decode buffer is sized
 * from that reduced resolution and freed internally.
 * Output buffer is allocated internally; release with tal_image_scale_buf_free().
 *
