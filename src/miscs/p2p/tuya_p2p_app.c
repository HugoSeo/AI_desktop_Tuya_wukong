...
#include "tuya_p2p_app.h"   /* TUYA_P2P_MEDIA_STREAM */
...
/* H264 subscriber: pushes frames to the P2P SDK. Subscribing also starts the
 * source's H264 stream (incl. DVP dual-stream switch); unsubscribing stops it. */
STATIC VOID __p2p_h264_cb(const VIDEO_FRAME_T *frame, VOID *ctx)
{
    (VOID)ctx;
    if (!frame || tuya_ipc_get_client_online_num() <= 0) {
        return;
    }
    MEDIA_FRAME_T mf = {
        .type      = frame->is_key_frame ? E_VIDEO_I_FRAME : E_VIDEO_PB_FRAME,
        .p_buf     = frame->data,
        .size      = frame->length,
        .pts       = tuya_p2p_misc_get_current_time_ms(),
        .timestamp = tuya_p2p_misc_get_current_time_ms(),
    };
    tuya_p2p_sdk_put_video_frame(&mf);
}
...
/* Audio transcode scratch buffers. Heap-allocated once in tuya_p2p_app_start()
 * so they cost PSRAM rather than BSS; this module has no stop/deinit, so they
 * are never freed — same lifetime as the fixed arrays they replace.
 * Sizing is unchanged; the derivation in the comment below says 800/400 (and an
 * older declaration to that effect is preserved above in history), so revisiting
 * it needs measured pcm_bytes / g711_bytes and is kept out of this change. */
#define P2P_TMP_PCM_SIZE   1600
#define P2P_G711_BUF_SIZE  1600

STATIC UCHAR_T *tmp_pcm  = NULL;
STATIC UCHAR_T *g711_buf = NULL;
INT_T tuya_ipc_app_audio_frame_put(VOID *data, INT_T len)
{
    if(tmp_pcm == NULL || g711_buf == NULL)
    {
        /* Distinct from "no client": the buffers are missing only when
         * tuya_p2p_app_start() failed to allocate them, and this runs per
         * audio frame — sharing the "No p2p client" message here would flood
         * the log and point diagnosis in the wrong direction. */
        TAL_PR_ERR("p2p audio transcode buffer not ready");
        return OPRT_OK;
    }
    ...
    if (rt != OPRT_OK) {
        ...
        // TAL_PR_DEBUG("put audio success: data size %d, 8k pcm bytes %d, g711 bytes %d", len, pcm_bytes, g711_bytes);
    }
       ...
       case MEDIA_STREAM_LIVE_VIDEO_START:
        {
            ...
            wukong_video_subscribe(VIDEO_FMT_H264, VIDEO_CONSUMER_P2P, __p2p_h264_cb, NULL);
            break;
        }
        ...
        case MEDIA_STREAM_LIVE_AUDIO_START:
        {
            ...
            wukong_ai_device_mode_switch(AI_DEVICE_MODE_P2P);
            TUYA_CALL_ERR_LOG(ty_publish_event(TUYA_P2P_MEDIA_STREAM, &msg));
            break;
        }
        ...
        case MEDIA_STREAM_SPEAKER_STOP:
        {
            ...
            /* Current validation policy: stop immediately on speaker stop.
             * tal_audio_output_stop() clears its internal ring buffer, so if
             * tail PCM truncation is observed, switch this to drain/idle stop. */
...
STATIC VOID __tuya_ipc_app_rev_audio_cb(IN INT_T device, IN INT_T channel, IN CONST MEDIA_AUDIO_FRAME_T *p_audio_frame)
{
    ...
    // 3. 将16K PCM送入播放接口
    wukong_audio_output_write_owned(WUKONG_AUDIO_OUTPUT_OWNER_P2P,
                                    (UINT8_T *)rev_pcm_16k,
                                    (UINT_T)out_bytes);
...
OPERATE_RET tuya_ipc_app_start()
{
    ...
    /* Video capability is taken from the video service at runtime: boards
     * without a video source (or camera disabled) report audio-only and the
     * call degrades to a plain intercom — no compile-time coupling to camera. */
    VIDEO_INPUT_CAPS_T vcaps = {0};
    if (wukong_video_available(VIDEO_FMT_H264) &&
        wukong_video_get_caps(&vcaps) == OPRT_OK) {
        sdkVar.media_info.stream_enable[E_IPC_STREAM_VIDEO_MAIN] = TRUE;
        sdkVar.media_info.video_fps[E_IPC_STREAM_VIDEO_MAIN] = vcaps.fps;
        sdkVar.media_info.video_gop[E_IPC_STREAM_VIDEO_MAIN] = vcaps.fps;
        sdkVar.media_info.video_bitrate[E_IPC_STREAM_VIDEO_MAIN] = TUYA_VIDEO_BITRATE_1M;
        sdkVar.media_info.video_width[E_IPC_STREAM_VIDEO_MAIN] = vcaps.width;
        sdkVar.media_info.video_height[E_IPC_STREAM_VIDEO_MAIN] = vcaps.height;
        sdkVar.media_info.video_freq[E_IPC_STREAM_VIDEO_MAIN] = 90000;
        sdkVar.media_info.video_codec[E_IPC_STREAM_VIDEO_MAIN] = TUYA_CODEC_VIDEO_H264;
    } else {
        sdkVar.media_info.stream_enable[E_IPC_STREAM_VIDEO_MAIN] = FALSE;
        TAL_PR_NOTICE("p2p: no H264 video source, audio-only call");
    }
...
{
    if (tmp_pcm == NULL) {
        tmp_pcm = tal_malloc(P2P_TMP_PCM_SIZE);
    }
    if (g711_buf == NULL) {
        g711_buf = tal_malloc(P2P_G711_BUF_SIZE);
    }
    if (tmp_pcm == NULL || g711_buf == NULL) {
        TAL_PR_ERR("[p2p] audio transcode buffer alloc failed (%d+%d bytes)",
                   P2P_TMP_PCM_SIZE, P2P_G711_BUF_SIZE);
        return OPRT_MALLOC_FAILED;
    }
    memset(tmp_pcm, 0, P2P_TMP_PCM_SIZE);
    memset(g711_buf, 0, P2P_G711_BUF_SIZE);
