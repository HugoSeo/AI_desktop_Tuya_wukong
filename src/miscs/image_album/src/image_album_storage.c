...
static void __album_storage_rollback_registers(void)
{
    ...
    /* SD/filesystem backend is app-assembled: the integrator calls
     * image_album_storage_sd_register_with_root() directly (e.g.
     * wukong_picture.c), since it needs a root path this layer does not own.
