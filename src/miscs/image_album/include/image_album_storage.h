 ...
 *        (e.g. "/sdcard"). The album directory becomes "<root>/<album_name>".
 *        This backend never mounts anything: the volume must be mounted (and
 *        kept mounted) by the integrator; ops degrade gracefully until then.
 * @param[in] root Device mount root; must outlive the album (string literal
 *                 or static). Must not be NULL/empty.
 * @return OPRT_OK on success
 */
