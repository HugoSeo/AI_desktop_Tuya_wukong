...
#define WUKONG_PICTURE_NAME_MAX_LEN ALBUM_FILENAME_MAX_LEN
    ...
    ***********************************************************/
    ...
    /**
     * @brief Move scan iterator so the next wukong_picture_get_next() loads the photo with this name
     * @param[in] name target picture filename
     * @return OPRT_OK on success, OPRT_COM_ERROR if album not open or name not found
     * @note Order-independent: scans by filename, so it works regardless of the
     *       scan sort order relative to the thumbnail list order.
     */
    OPERATE_RET wukong_picture_seek_to_name(const char *name);

    /**
     * @brief Move the scan iterator to a named photo and return its scan index
     * @param[in] name target picture filename
     * @param[out] zero_based_index index in the current scan order (may be NULL)
     * @return OPRT_OK on success, OPRT_COM_ERROR if the album is not open or the name is not found
     */
