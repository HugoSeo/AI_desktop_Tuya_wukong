...
/**
 * @brief Registers backends and creates the album. Idempotent — called from
 *        two places (the base_event callback path and the wukong_picture_init()
 *        catch-up path below), both of which are made to run on WORKQ_SYSTEM
 *        (see wukong_picture_init()), so the two invocations are serialized
 *        on a single worker thread rather than racing on separate ones. That
 *        serialization is what makes the bare sg_setup_done check below
 *        sufficient as an idempotency guard, no lock needed.
 * @return OPRT_OK on success
 */
STATIC OPERATE_RET __picture_album_setup(VOID_T)
{
    OPERATE_RET rt = OPRT_OK;
    static BOOL_T sg_setup_done = FALSE;
    ...
    defined(ENABLE_IMAGE_ALBUM_STORAGE_SD) && (ENABLE_IMAGE_ALBUM_STORAGE_SD)
    TUYA_CALL_ERR_LOG(image_album_storage_sd_register_with_root(WUKONG_STORAGE_ROOT));
#endif
    ...
    defined(ENABLE_IMAGE_ALBUM_STORAGE_SD) && (ENABLE_IMAGE_ALBUM_STORAGE_SD)
/**
 * @brief Storage-ready event callback: run the deferred album setup
 * @param[in] data unused
 * @return OPRT_OK on success
 */
STATIC OPERATE_RET __picture_on_storage_ready(VOID_T *data)
{
    (VOID)data;
    return __picture_album_setup();
}
 ...
 *        running inline on whatever thread calls wukong_picture_init().
 */
STATIC VOID_T __picture_catchup_on_workq(VOID_T *data)
{
    (VOID)data;
    __picture_album_setup();
}
#endif
...
#if defined(WUKONG_STORAGE_ENABLE) && (WUKONG_STORAGE_ENABLE == 1) && \
    defined(ENABLE_IMAGE_ALBUM_STORAGE_SD) && (ENABLE_IMAGE_ALBUM_STORAGE_SD)
    /* Album recovery must see the mounted volume: subscribe first, then
     * catch up if storage won the race. */
    OPERATE_RET rt = ty_subscribe_event(EVENT_WUKONG_STORAGE_READY, "wk_pic",
                                        __picture_on_storage_ready, SUBSCRIBE_TYPE_NORMAL);
    if (rt != OPRT_OK) {
        return rt;
    }
    /* Do NOT call __picture_album_setup() directly here: this function can
     * run on any caller thread while the event callback runs on WORKQ_SYSTEM,
     * and both could pass the sg_setup_done check before either sets it,
     * running image_album_init() (unsynchronized find-then-insert) concurrently
     * — duplicate album nodes / backend registrations, leaked handle. Hopping
     * the catch-up onto WORKQ_SYSTEM too serializes it against the event
     * callback instead. */
    if (wukong_storage_ready()) {
        if (tal_workq_schedule(WORKQ_SYSTEM, __picture_catchup_on_workq, NULL) != OPRT_OK) {
            PR_WARN("wukong_picture: catch-up schedule failed, album may not init");
        }
    }
    return OPRT_OK;
#else
    /* Memory-only album (no persistent backend) — set up immediately. */
    return __picture_album_setup();
#endif
}
...
OPERATE_RET wukong_picture_seek_to_name_index(const char *name, uint32_t *zero_based_index)
{
    uint32_t i;
    ...
    /* Walk the scan order by metadata only (no JPEG read) to find the 1-based
     * position of the named photo, then position the cursor there so the next
     * wukong_picture_get_next() returns it. */
    if (image_album_scan_seek(sg_picture_ctx.album_scan_hdl, 0U) != OPRT_OK) {
        return OPRT_COM_ERROR;
    }

    for (i = 0; i < sg_picture_ctx.scan_img_count; i++) {
        ALBUM_IMAGE_ITEM_T item;
        memset(&item, 0x00, sizeof(ALBUM_IMAGE_ITEM_T));
        if (image_album_scan_next(sg_picture_ctx.album_scan_hdl, &item) != OPRT_OK) {
            break;
        }
        if (strncmp(item.filename, name, ALBUM_FILENAME_MAX_LEN) == 0) {
            OPERATE_RET rt = wukong_picture_seek_to_photo(i + 1U);
            if (rt == OPRT_OK && zero_based_index != NULL) {
                *zero_based_index = i;
            }
            return rt;
        }
...
OPERATE_RET wukong_picture_seek_to_name(const char *name)
{
    return wukong_picture_seek_to_name_index(name, NULL);
}
                                          ...
                                          WUKONG_PICTURE_THUMB_LIST_T *list)
{
    ...
    /* A batch may contain valid thumbnails even when one item failed. Keep the
     * successful entries instead of leaking/discarding the whole batch. */
    if (rt != OPRT_OK && batch.count == 0) {
