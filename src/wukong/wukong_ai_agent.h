...
typedef struct {
    ...
    CONST CHAR_T *chat_id;        /**< Session ID (routing). */
    CONST CHAR_T *channel;        /**< Source channel name (routing). */
} WUKONG_AI_EVENT_T;
...
/* --- Facade API (Provider-backed, replaces direct SDK calls in mode layer) --- */
...
/**
 * @brief Send one serialized MCP (JSON-RPC) message out through the active provider.
 *
 * Unified outbound entry for the MCP server: routes to the current provider's
 * ops->mcp_send (polymorphic — follows cur_provider, no binding/switching needed).
 * Returns OPRT_COM_ERROR when the active provider has no MCP channel.
 *
 * @param[in] sid     Session id (echoed by transports that need it).
 * @param[in] eid     Event id (may be empty for transports without one, e.g. cube).
 * @param[in] message Null-terminated serialized JSON-RPC message. Must not be NULL.
 */
