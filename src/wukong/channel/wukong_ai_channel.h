...
#define WUKONG_CHAN_BOARD    "board"    /* on-device hardware (mic/speaker/screen) */
#define WUKONG_CHAN_WECHAT   "wechat"
#define WUKONG_CHAN_CLI      "cli"

#define WUKONG_CHATID_DEFAULT  "default"  /* chat_id for single-session channels */

/* --- Channel capability flags --- */

/** IM channel: AI reply is sent back via channel_output → chan->send().
 *  If not set (board channel): reply is delivered via wukong_ai_event_notify only. */
#define WUKONG_CHAN_FLAG_IM   (1U << 0)

/* --- Message flags (additional, complement WUKONG_AI_MSG_FLAG_* in provider.h) --- */

/** Final fragment of a text stream — channel send() should flush the reply. */
#define WUKONG_AI_MSG_FLAG_EOF  (1U << 3)

/* --- Channel Descriptor (const, ROM-safe) --- */

typedef struct {
    CONST CHAR_T *name;
    UINT32_T      flags;           /* WUKONG_CHAN_FLAG_* bitmask */
 ...
 * @param[in] text     Text fragment; may be NULL when len is 0.
