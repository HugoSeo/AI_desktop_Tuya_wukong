...
/* Reply accumulator lives further down (next to the code that uses it); its
 * backing buffer is allocated here at init. */
STATIC OPERATE_RET __channel_reply_buf_init(VOID_T);
...
/* ---------------------------------------------------------------------------
 * Reply stream accumulator — collects NLG text fragments from provider and
 * dispatches the complete message to the active IM channel on STREAM_STOP.
 * Shared by all IM channels (wechat, cli, telegram, ...).
 * --------------------------------------------------------------------------- */
/* Holds a full assembled reply before dispatch. LLM replies (e.g. Claw) can be
 * several KB; the WeChat channel re-splits into <=4000-byte messages downstream. */
#define CHANNEL_REPLY_BUF_MAX  8192

typedef struct {
    /** Accumulator, CHANNEL_REPLY_BUF_MAX bytes. Heap-allocated once in
     *  wukong_ai_channel_init() so it costs PSRAM rather than BSS; the channel
     *  layer has no deinit, so it is never freed — same lifetime as the fixed
     *  array it replaces. */
    CHAR_T *buf;
    UINT_T  len;
    CHAR_T  channel[32];
    CHAR_T  chat_id[64];
} __channel_reply_buf_t;

STATIC __channel_reply_buf_t s_channel_reply_buf;

STATIC OPERATE_RET __channel_reply_buf_init(VOID_T)
{
    if (s_channel_reply_buf.buf != NULL) {
        return OPRT_OK;
    }
    s_channel_reply_buf.buf = tal_malloc(CHANNEL_REPLY_BUF_MAX);
    if (s_channel_reply_buf.buf == NULL) {
        PR_ERR("channel reply buf alloc %u bytes failed", (UINT_T)CHANNEL_REPLY_BUF_MAX);
        return OPRT_MALLOC_FAILED;
    }
    s_channel_reply_buf.buf[0] = '\0';
    return OPRT_OK;
}

STATIC VOID_T __channel_reply_buf_reset(VOID_T)
{
    s_channel_reply_buf.len        = 0;
    if (s_channel_reply_buf.buf != NULL) {
        s_channel_reply_buf.buf[0] = '\0';
    }
    s_channel_reply_buf.channel[0] = '\0';
    s_channel_reply_buf.chat_id[0] = '\0';
}

STATIC VOID_T __channel_reply_buf_feed(CONST CHAR_T *data, UINT_T len)
{
    UINT_T rem, cp;

    if (data == NULL || len == 0U || s_channel_reply_buf.buf == NULL) {
        return;   /* buf is NULL only if channel init failed */
    }
    rem = (UINT_T)(CHANNEL_REPLY_BUF_MAX - s_channel_reply_buf.len - 1U);
    cp  = (len < rem) ? len : rem;
    if (cp == 0U) {
        PR_WARN("channel reply buf overflow (%u), dropped %u bytes",
                CHANNEL_REPLY_BUF_MAX, len);
        return;
    }
    if (cp < len) {
        PR_WARN("channel reply buf truncated: %u/%u bytes", cp, len);
    }
    memcpy(s_channel_reply_buf.buf + s_channel_reply_buf.len, data, cp);
    s_channel_reply_buf.len += cp;
    s_channel_reply_buf.buf[s_channel_reply_buf.len] = '\0';
}

STATIC VOID_T __channel_reply_buf_flush(VOID_T)
{
    WUKONG_AI_MSG_T out;

    if (s_channel_reply_buf.len == 0U || s_channel_reply_buf.channel[0] == '\0') {
        __channel_reply_buf_reset();
        return;
    }

    out.type     = WUKONG_AI_MSG_TYPE_TEXT;
    out.data     = (CONST BYTE_T *)s_channel_reply_buf.buf;
    out.data_len = s_channel_reply_buf.len;
    out.flags    = 0;
    PR_INFO("channel reply flush: ch=%s id=%s len=%u",
            s_channel_reply_buf.channel, s_channel_reply_buf.chat_id, s_channel_reply_buf.len);
    wukong_ai_channel_output(s_channel_reply_buf.channel, s_channel_reply_buf.chat_id, &out);
    __channel_reply_buf_reset();
}

/**
 * @brief Collect NLG text stream events and dispatch the complete reply
 *        to the active IM channel on STREAM_STOP.
 *
 * STREAM_START resets the accumulator and records the target channel/chat_id.
 * STREAM_DATA appends fragments.  STREAM_STOP flushes via channel_output().
 * STREAM_ABORT discards the buffer.
 *
 * @param[in] type     TEXT_STREAM_START / DATA / STOP / ABORT.
 * @param[in] text     Text fragment; may be NULL when text_len is 0.
 * @param[in] text_len Fragment length in bytes.
 * @return TRUE if active channel has WUKONG_CHAN_FLAG_IM and event was consumed.
 */
BOOL_T wukong_ai_channel_dispatch_reply(WUKONG_AI_EVENT_TYPE_E type,
                                        CONST CHAR_T *text,
                                        UINT_T text_len)
{
    CONST WUKONG_AI_ACTIVE_CHAN_T *active = NULL;
    CONST WUKONG_AI_CHAN_T *chan = NULL;

    active = wukong_ai_channel_get_active(NULL);
    if (active == NULL) {
        return FALSE;
    }
    chan = wukong_ai_channel_find(active->channel);
    if (chan == NULL || !(chan->flags & WUKONG_CHAN_FLAG_IM)) {
        return FALSE;
    }
    ...
    case WUKONG_AI_EVENT_TEXT_STREAM_START:
        __channel_reply_buf_reset();
        strncpy(s_channel_reply_buf.channel, active->channel, sizeof(s_channel_reply_buf.channel) - 1);
        strncpy(s_channel_reply_buf.chat_id, active->chat_id, sizeof(s_channel_reply_buf.chat_id) - 1);
        __channel_reply_buf_feed(text, text_len);
        break;
