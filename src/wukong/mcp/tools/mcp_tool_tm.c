...
STATIC OPERATE_RET __register_alarm_tools(VOID)
{
    ...
    rt = MCP_TOOL_ADD(
        ...
        "`repeat_type`: 0=once (default; needs year+month+day+hour+minute, future), "
        ...
        "the time is taken), `alarm_time_in_the_past`, `invalid_time`, "
    ...
    return MCP_TOOL_ADD(
        "device_alarm_query",
        "Query LOCAL ALARMS only (闹钟, ringtone-based; one-time and recurring). "
        ...
        "`message` strings such as '提醒我喝水', 'remind me ...' or anything "
        "containing '提醒/日程/remind/schedule' MUST NOT be reinterpreted as a "
        "reminder/schedule item; they are still alarms. "
        "This tool NEVER returns spoken reminders/schedules (提醒/日程); use "
        "`device_schedule_query` for those. "
        "Reply rules: when the user explicitly asks about '闹钟'/'alarm', call "
        "ONLY this tool and describe the results using the word '闹钟'; do NOT "
        "use '提醒' or '日程' in the spoken reply, even if a `message` contains "
        "those words. Only call BOTH this tool and `device_schedule_query` when "
        "the user uses an ambiguous term that covers both (e.g. '我有什么待办').",
        __query_alarm, NULL
    );
}
...
STATIC OPERATE_RET __register_counter_tools(VOID)
{
    ...
    rt = MCP_TOOL_ADD(
        ...
        MCP_SCHEMA_INT_OPT_RANGE("minute_duration", "Minutes (create only)", 0, 60),
...
STATIC OPERATE_RET __register_schedule_tools(VOID)
{
    ...
    rt = MCP_TOOL_ADD(
        ...
        MCP_SCHEMA_STR("id", "Reminder identifier (unique on add; required for all operations)"),
