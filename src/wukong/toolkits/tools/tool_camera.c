                                 ...
                                 VOID *user_data)
{
    ...
    TUYA_CALL_ERR_LOG(tuya_camera_snapshot(&image_data, &image_size, 3000));
    if (image_data == NULL || image_size == 0) {
        TAL_PR_ERR("snapshot failed");
    ...
    *out_content = ty_cJSON_CreateArray();
    if (*out_content)
        ty_cJSON_AddItemToArray(*out_content,
            wukong_tool_make_image(MCP_MIME_JPEG, image_data, image_size));
...
OPERATE_RET tool_camera_init(VOID)
{
    return WUKONG_TOOL_ADD(
        ...
        __take_photo, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT
    );
}
