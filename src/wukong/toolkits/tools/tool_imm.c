 ...
 * tool_imm_register_platform() by external modules.
 *
...
OPERATE_RET tool_imm_register_platform(IMM_PLATFORM_E platform,
                                            IMM_SEND_CB send_cb,
                                            IMM_QUERY_CB query_cb)
{
                               ...
                               VOID *user_data)
{
        ...
        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Missing required parameters: platform, contact, message"));
    ...
    if (platform >= IMM_PLATFORM_MAX) {
        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Unsupported platform"));
        return OPRT_COM_ERROR;
    }
    ...
    if (!s_platforms[platform].send_cb) {
        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Platform not integrated yet"));
        return OPRT_COM_ERROR;
    }
    ...
    *out_content = ty_cJSON_CreateArray();
    if (*out_content)
        ty_cJSON_AddItemToArray(*out_content,
            wukong_tool_make_text(rt == OPRT_OK ? "true" : "false"));
    return rt;
}
                                ...
                                VOID *user_data)
{
    ...
    if (!platform_str) {
        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Missing required parameter: platform"));
        return OPRT_COM_ERROR;
    }
    ...
    *out_content = ty_cJSON_CreateArray();
    if (*out_content) {
        if (rt == OPRT_OK && result_json) {
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text(result_json));
        } else {
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Query failed"));
        }
    }

    BOOL_T ok = (rt == OPRT_OK && result_json != NULL);
    if (result_json)
...
OPERATE_RET tool_imm_init(VOID)
{
    OPERATE_RET rt;
    ...
    rt = WUKONG_TOOL_ADD(
        ...
        __imm_send, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT,
        TOOL_SCHEMA_STR("platform",
            "Target IM platform: 'wechat', 'feishu', 'discord', 'whatsapp'"),
        TOOL_SCHEMA_STR("contact",
            "Contact name or ID to send the message to"),
        TOOL_SCHEMA_STR("message",
    ...
    rt = WUKONG_TOOL_ADD(
        ...
        __imm_query, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT,
        TOOL_SCHEMA_STR("platform",
            "Target IM platform: 'wechat', 'feishu', 'discord', 'whatsapp'"),
        TOOL_SCHEMA_STR_OPT("contact",
            "Contact name or ID to filter messages (omit for all contacts)"),
        TOOL_SCHEMA_INT_OPT_RANGE("count",
