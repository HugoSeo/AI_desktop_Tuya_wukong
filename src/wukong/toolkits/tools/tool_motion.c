    ...
    *out_content = ty_cJSON_CreateArray();
    if (*out_content)
        ty_cJSON_AddItemToArray(*out_content,
            wukong_tool_make_text(ok ? "true" : "false"));

    TAL_PR_DEBUG("__set_motion_control exit");
    return OPRT_OK;
}

OPERATE_RET tool_motion_init(VOID)
{
    return WUKONG_TOOL_ADD(
        ...
        __set_motion_control, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT,
        TOOL_SCHEMA_INT_RANGE("motion_mode",
            "Motion mode (0=left, 1=right, 2=appoint angle, 3=clockwise, 4=counterclockwise, 5=point move, 6=reset, 7=stop)",
            0, 7),
        TOOL_SCHEMA_INT_RANGE("rotate_value",
