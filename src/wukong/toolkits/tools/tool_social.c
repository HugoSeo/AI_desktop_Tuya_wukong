 ...
 * tool_social_register_platform() by external modules.
 *
...
OPERATE_RET tool_social_register_platform(SOCIAL_PLATFORM_E platform,
                                               SOCIAL_FEED_CB feed_cb)
{
    ...
    *out_content = ty_cJSON_CreateArray();
    if (*out_content) {
        if (rt == OPRT_OK && result_json) {
            ...
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Failed to fetch feed"));
        }
    }
...
OPERATE_RET tool_social_init(VOID)
{
    return WUKONG_TOOL_ADD(
        ...
        __social_feed, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT,
        TOOL_SCHEMA_STR("platform",
            "Social media platform: 'weibo', 'youtube', 'x', 'tiktok', 'bilibili'"),
        TOOL_SCHEMA_STR_OPT("feed_type",
