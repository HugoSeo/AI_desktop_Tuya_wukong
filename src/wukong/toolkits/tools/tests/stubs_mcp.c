...
unsigned int g_last_update_alarm_repeat_type = 0;
unsigned int g_last_alarm_add_repeat_type = 0;
...
static char g_stub_reminder_id[64] = {0};
...
OPERATE_RET tal_time_get_time_zone_seconds(INT_T *tz_sec)
{
    ...
    *tz_sec = TEST_TIMEZONE_OFFSET_SEC;
    return OPRT_OK;
}

/* ---- tool dispatch helpers ---- */
static WUKONG_TOOL_HANDLER_CB find_handler(const char *name)
{
    int i = 0;
...
{
    WUKONG_TOOL_HANDLER_CB handler = find_handler(name);
    ty_cJSON *out = NULL;
                                ...
                                CONST CHAR_T *alarm_id)
{
    ...
    if (alarm_cfg != NULL) {
        ...
        g_last_alarm_add_repeat_type = (unsigned int)alarm_cfg->repeat_type;
                                   ...
                                   CONST WUKONG_TM_ALARM_CFG_T *alarm_cfg)
{
    ...
    if (alarm_cfg != NULL) {
        ...
        g_last_update_alarm_repeat_type = (unsigned int)alarm_cfg->repeat_type;
        strncpy(g_last_alarm_message, alarm_cfg->message,
                sizeof(g_last_alarm_message) - 1);
    }
                                   ...
                                   WUKONG_TM_REMINDER_CFG_T *reminder_cfg)
{
    ...
    if (reminder_id != NULL && g_stub_reminder_id[0] != '\0' &&
        strcmp(reminder_id, g_stub_reminder_id) != 0) {
        return OPRT_NOT_FOUND;
    }
