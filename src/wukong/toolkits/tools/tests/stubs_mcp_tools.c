...
static WUKONG_TM_ALARM_CFG_T s_stub_alarm_cfg = {0};
static BOOL_T s_stub_alarm_cfg_valid = FALSE;

VOID test_reset_alarm_stub(VOID)
{
    s_stub_alarm_cfg_valid = FALSE;
    memset(&s_stub_alarm_cfg, 0, sizeof(s_stub_alarm_cfg));
}
static WUKONG_TM_ALARM_CFG_T s_last_alarm_add_cfg = {0};
static BOOL_T s_last_alarm_add_cfg_valid = FALSE;

OPERATE_RET wukong_tm_alarm_add(CONST WUKONG_TM_ALARM_CFG_T *alarm_cfg, CONST CHAR_T *alarm_id)
{
    (void)alarm_id;
    if (alarm_cfg != NULL) {
        s_stub_alarm_cfg = *alarm_cfg;
        s_stub_alarm_cfg_valid = TRUE;
        s_last_alarm_add_cfg = *alarm_cfg;
        s_last_alarm_add_cfg_valid = TRUE;
    }
    return OPRT_OK;
}
...
{
    (void)alarm_id;
    if (!s_stub_alarm_cfg_valid || alarm_cfg == NULL) {
        return OPRT_NOT_FOUND;
    }
    *alarm_cfg = s_stub_alarm_cfg;
    return OPRT_OK;
}
...
OPERATE_RET tal_time_get_time_zone_seconds(INT_T *tz_sec)
{
    ...
    *tz_sec = TM_TOOLS_TEST_TZ_OFFSET_SEC;  /* +08:00 = 28800 */
    return OPRT_OK;
}
...
void test_set_reminder_snapshot(const char *reminder_id, TIME_T start_time, const char *message)
{
    ...
    g_stub_reminder_cfg.repeat_type = WUKONG_TM_REPEAT_ONCE;
    g_stub_reminder_cfg.hour = 0;
    g_stub_reminder_cfg.minute = 0;
    g_stub_reminder_cfg.weekday_mask = 0;
    g_stub_reminder_cfg.month_day = 0;
...
{
    return s_last_alarm_add_cfg_valid;
}

WUKONG_TM_REPEAT_TYPE_E test_last_alarm_add_repeat_type(void)
{
    return s_last_alarm_add_cfg.repeat_type;
}

BOOL_T test_last_alarm_add_enabled(void)
{
    return s_last_alarm_add_cfg.enabled;
}
