                                       ...
                                       VOID *user_data)
{
        ...
        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Missing action parameter"));
        return OPRT_COM_ERROR;
    }

    TAL_PR_DEBUG("playback control action: %s", action);

    /* Loop mode changes: set play mode directly rather than through the event
     * system, whose mode handlers are empty stubs for these events. */
    if (strcmp(action, "single_loop") == 0 ||
        strcmp(action, "sequential_loop") == 0 ||
        strcmp(action, "no_loop") == 0 ||
        strcmp(action, "shuffle") == 0) {

        WK_PLAY_MODE_E mode = WK_PLAY_MODE_SEQUENCE;
        if (strcmp(action, "single_loop") == 0)
            mode = WK_PLAY_MODE_SINGLE_LOOP;
        else if (strcmp(action, "sequential_loop") == 0)
            mode = WK_PLAY_MODE_LIST_LOOP;
        else if (strcmp(action, "shuffle") == 0)
            mode = WK_PLAY_MODE_SHUFFLE;

        wukong_playback_ctrl_set_play_mode(mode);

        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("true"));
        return OPRT_OK;
    }

    /* Event group: resume / pause / replay */
    ...
    if (strcmp(action, "next") == 0 || strcmp(action, "prev") == 0) {
        ...
        if (rt == OPRT_OK) {
            ...
            *out_content = ty_cJSON_CreateArray();
            if (*out_content) {
                ty_cJSON_AddItemToArray(*out_content,
                    wukong_tool_make_text(out ? out : "{\"ok\":true}"));
            }
            if (out) {
    ...
    if (strcmp(action, "auto_play_next_on") == 0 ||
        ...
        *out_content = ty_cJSON_CreateArray();
        if (*out_content) {
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text(enable ? "{\"auto_play_next\":true}"
    ...
    *out_content = ty_cJSON_CreateArray();
    if (*out_content) {
        ty_cJSON_AddItemToArray(*out_content,
            wukong_tool_make_text("Unknown action"));
    }
    return OPRT_COM_ERROR;
}
                                      ...
                                      VOID *user_data)
{
    ...
    /* Include play mode so the AI can answer "what loop mode am I in?" */
    CONST CHAR_T *mode_str = "sequence";
    WK_PLAY_MODE_E mode = wukong_playback_ctrl_get_play_mode();
    if (mode == WK_PLAY_MODE_SINGLE_LOOP)
        mode_str = "single_loop";
    else if (mode == WK_PLAY_MODE_LIST_LOOP)
        mode_str = "list_loop";
    else if (mode == WK_PLAY_MODE_SHUFFLE)
        mode_str = "shuffle";
    ty_cJSON_AddStringToObject(result, "play_mode", mode_str);
    ...
    *out_content = ty_cJSON_CreateArray();
    if (*out_content)
        ty_cJSON_AddItemToArray(*out_content,
            wukong_tool_make_text(json_str ? json_str : "{}"));
    if (json_str)
                                   ...
                                   VOID *user_data)
{
    ...
    if (rt != OPRT_OK || !result) {
        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Music search timed out or failed"));
        return OPRT_COM_ERROR;
    }
     ...
     * NOTE: first_name/first_artist_str borrow pointers into `result`, so it
     * must stay alive until the summary has copied them; delete it afterwards. */
    ...
    ty_cJSON_Delete(result);   /* safe now: summary holds copies of first_name/artist */
                                    ...
                                    VOID *user_data)
{
    ...
    if (rt != OPRT_OK || !json) {
        *out_content = ty_cJSON_CreateArray();
        if (*out_content)
            ty_cJSON_AddItemToArray(*out_content,
                wukong_tool_make_text("Failed to retrieve playlist"));
        return OPRT_COM_ERROR;
    }
 ...
 */
OPERATE_RET tool_playback_init(VOID)
{
    OPERATE_RET rt;
    ...
    rt = WUKONG_TOOL_ADD(
        ...
        "- 'single_loop' / 'sequential_loop' / 'no_loop' / 'shuffle': Change the loop mode.\n"
        "- 'auto_play_next_on' / 'auto_play_next_off': Enable or disable automatic "
        "playback of the next song when the current one finishes.",
        __playback_control, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT,
        TOOL_SCHEMA_STR("action",
            "Playback action: "
            "'next' = next song, "
            "'prev' = previous song, "
    ...
    rt = WUKONG_TOOL_ADD(
        ...
        "auto_play_next (true/false) and play_mode "
        "(sequence/single_loop/list_loop/shuffle).",
        __playback_status, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT
    );
    if (rt != OPRT_OK) {
        return rt;
    }
    ...
    rt = WUKONG_TOOL_ADD(
        ...
        __music_search, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT,
        TOOL_SCHEMA_STR_OPT("keyword", "General search keyword"),
        TOOL_SCHEMA_STR_OPT("tag", "Music category tag, e.g. 'children','pop'"),
        TOOL_SCHEMA_STR_OPT("name", "Song title"),
        TOOL_SCHEMA_STR_OPT("artist", "Artist name"),
        TOOL_SCHEMA_STR_OPT("album_name", "Album name"),
        TOOL_SCHEMA_INT_OPT("offset", "Page offset, default 0"),
        TOOL_SCHEMA_INT_OPT_RANGE("limit", "Page size, default 5, max 5", 1, MUSIC_SEARCH_LIST_LIMIT_CAP)
    );
    if (rt != OPRT_OK) {
        return rt;
    }
    ...
    rt = WUKONG_TOOL_ADD(
        ...
        __playlist_list, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT
    );
    if (rt != OPRT_OK) {
        return rt;
    }
    ...
    rt = WUKONG_TOOL_ADD(
        ...
        __playlist_clear, NULL, WUKONG_TOOL_MCP | WUKONG_TOOL_AGENT
    );
