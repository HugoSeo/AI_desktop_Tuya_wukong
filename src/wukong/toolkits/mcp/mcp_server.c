...
{
    /* Outbound goes to whichever cloud provider is active (polymorphic via cur_provider).
     * MCP core stays cloud-SDK agnostic — it only knows the provider facade. */
    wukong_ai_agent_mcp_send(sid, eid, message);
}
                                     ...
                                     CONST CHAR_T *id, ty_cJSON *result)
{
    ...
    /* id is a raw JSON token (number or string) — emit verbatim to preserve type */
    ty_cJSON_AddRawToObject(response, "id", id);
    if (result)
                                    ...
                                    CONST CHAR_T *msg)
{
    ...
    ty_cJSON_AddStringToObject(response, "jsonrpc", "2.0");
    if (id)
        ty_cJSON_AddRawToObject(response, "id", id);   /* raw token preserves number/string type */
    else
                                       ...
                                       CONST ty_cJSON *msg, VOID *user_data)
{
    ty_cJSON *node;
    CONST CHAR_T *method;
    CHAR_T *id_raw = NULL;   /* raw JSON token of "id" (number or string), freed before return */
    ty_cJSON *params;
    ...
    /* JSON-RPC 2.0 id may be a string OR a number (xiaozhi/mcp.md use numeric ids).
     * Carry it as its raw JSON token so the response echoes the original type. */
    node = ty_cJSON_GetObjectItem(msg, "id");
    if (node && (ty_cJSON_IsString(node) || ty_cJSON_IsNumber(node)))
        id_raw = ty_cJSON_PrintUnformatted(node);
    ...
    for (UINT_T i = 0; i < NUM_ROUTES; i++) {
        if (strcmp(method, s_routes[i].method) == 0) {
            rt = s_routes[i].handler(sid, eid, params, id_raw);
            routed = TRUE;
            break;
        }
    }
    if (!routed) {
        ...
        rt = mcp_server_reply_error(sid, eid, id_raw, MCP_ERR_METHOD_NOT_FOUND,
                                     "Method not found");
    }

    /* Handlers use id_raw synchronously (immediate reply) or strdup it (async tool
     * path) before returning, so it is safe to release here. */
    ty_cJSON_FreeBuffer(id_raw);
    return rt;
}
...
OPERATE_RET mcp_server_init(CONST CHAR_T *name, CONST CHAR_T *version)
{
    ...
    /* Inbound is wired by each provider (it registers/feeds mcp_server_handle_message
     * from its own receive path) — the core no longer binds any transport. */
...
VOID mcp_server_destroy(VOID)
{
    ...
    /* Tool registry lifetime is independent of the MCP transport (agent/fc
     * keep using it after mcp_server_destroy); no cap_destroy call here
     * — see toolkits/wukong_tool.h mcp_tools_cap_destroy(). */
#if MCP_ENABLE_RESOURCES
