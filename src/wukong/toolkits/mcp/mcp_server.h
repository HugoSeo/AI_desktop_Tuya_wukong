 ...
 * Transport-agnostic: the active provider supplies outbound (ops->mcp_send, reached
 * via wukong_ai_agent_mcp_send) and feeds inbound (mcp_server_handle_message).
 */
...
/**
 * Main message handler / inbound entry. A provider hands a parsed JSON-RPC 2.0
 * message here for routing (see the wukong_mcp_recv alias in wukong_mcp.h).
 */
