...
typedef struct {
    ...
    TIMER_ID timer;
    /** Mutex protecting the job table and timer (re)arming. */
    MUTEX_HANDLE mutex;
    /** In-memory job table, WUKONG_CRON_MAX_JOBS entries. Heap-allocated in
     *  wukong_cron_init() and freed in wukong_cron_deinit(), so it costs PSRAM
     *  rather than BSS; it exists for the whole lifetime of the module, exactly
     *  like the fixed array it replaces. */
    WUKONG_CRON_JOB_T *jobs;
} WUKONG_CRON_CTX_T;

STATIC WUKONG_CRON_CTX_T s_cron_ctx;

#define WUKONG_CRON_JOBS_BYTES  (sizeof(*s_cron_ctx.jobs) * WUKONG_CRON_MAX_JOBS)
...
STATIC VOID __cron_timer_cb(TIMER_ID timer_id, VOID_T *arg)
{
    ...
    tal_mutex_lock(s_cron_ctx.mutex);
    ...
    tal_mutex_unlock(s_cron_ctx.mutex);
    if (rt != OPRT_OK) {
...
OPERATE_RET wukong_cron_init(VOID)
{
    ...
    s_cron_ctx.jobs = tal_malloc(WUKONG_CRON_JOBS_BYTES);
    if (s_cron_ctx.jobs == NULL) {
        TAL_PR_ERR("cron -> job table alloc %d bytes failed", (INT_T)WUKONG_CRON_JOBS_BYTES);
        return OPRT_MALLOC_FAILED;
    }
    memset(s_cron_ctx.jobs, 0, WUKONG_CRON_JOBS_BYTES);
    ...
    rt = tal_mutex_create_init(&s_cron_ctx.mutex);
    if (rt != OPRT_OK) {
...
err_free_jobs:
    tal_free(s_cron_ctx.jobs);
    s_cron_ctx.jobs = NULL;
    return rt;
}
...
OPERATE_RET wukong_cron_deinit(VOID)
{
    ...
    if (s_cron_ctx.mutex) {
        tal_mutex_release(s_cron_ctx.mutex);
