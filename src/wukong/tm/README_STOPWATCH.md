...
> 与**倒计时**不同：秒表为**单调递增**，**没有**内置「到时自动结束」；模块也**不会**下发 `TICK` / `FINISH` 类进度事件。需要界面上的连续读数时，请配合 **`wukong_tm_stopwatch_query()`**、MCP **`device_stopwatch_timer_set` `operation=5`（query）** 或自有定时器轮询。C API 快照提供毫秒读数，MCP 与事件 TLV 继续使用兼容的整秒读数。
...
- **纯本地**：不注册 Cron，以不受校时影响的 `tal_system_get_millisecond()` 单调时钟累计毫秒；读数只包含从本次 `start` 起的**运行态**时长，**暂停时冻结**（见 §2.1）。
- **事件载荷**：
...
| `wukong_tm_stopwatch_query(state)` | 查询快照：`active`、`paused`、`elapsed_ms`、`elapsed_sec` |

### 2.1 `WUKONG_TM_STOPWATCH_STATE_T` 的时间字段

- **`elapsed_ms`**：权威跑表读数，单位毫秒。**仅在运行态**随单调时钟增加；暂停时保持为暂停瞬间的累计值，墙上暂停时长不会计入。
- **`elapsed_sec`**：兼容字段，始终等于 `elapsed_ms / 1000` 向下取整。既有 MCP JSON 和 `WUKONG_TM_TAG_STOPWATCH_ELAPSED_SEC` TLV 的字段与字节格式不变。
...
| 值 | 符号 | 含义 | 典型处理建议 |
|----|------|------|----------------|
| 0 | `WUKONG_TM_TIMER_OPR_START` | **`start` 成功**，新会话开始 | 秒表 UI 归零并开始跑表动画；可立即 `query` 得 `elapsed_ms == 0` |
...
因此：在收到 **`STOP` 或 `RESET`** 的回调里，**仍可调用** `wukong_tm_stopwatch_query()`，此时会话尚未销毁，可读到最终 `elapsed_ms` 与兼容的 `elapsed_sec`；回调返回后实例即清空，再次 `query` 会得到 `OPRT_NOT_FOUND`。

### 3.4 平滑 UI 刷新

因无 `TICK` 事件，常见做法是：

1. 在收到 `START` / `RESUME` 后启动受页面生命周期约束的 UI 刷新（如每 30ms～100ms）调用 `query`，用 `elapsed_ms` 格式化百分秒；`PAUSE` 后停止递增或仅刷新一次；`STOP`/`RESET` 后停止刷新。
2. 或仅在用户可见时轮询，以降低功耗。
...
- 若需要「到某一刻自动停表」，需在应用层根据 `query` 的 `elapsed_ms`（或兼容的 `elapsed_sec`）自行判断并调用 `stop()`（或只停 UI，视需求而定）。
...
- **`query`（5）** 成功时返回：`{"success":true,"active":...,"paused":...,"elapsed_sec":...}`；无活跃秒表为 `{"success":true,"active":false}`。MCP 保持整秒协议，毫秒读数仅由 C API 快照提供。
