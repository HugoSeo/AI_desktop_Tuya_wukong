...
/* fire observer test hooks (defined in this test file below) */
extern int g_fire_cb_count;
extern WUKONG_TM_FIRE_EVENT_E g_fire_cb_last_event;
extern char g_fire_cb_last_id[WUKONG_TM_ALARM_ID_LEN + 1];
void test_fire_observer(const WUKONG_TM_FIRE_INFO_T *info);

int g_fire_cb_count = 0;
WUKONG_TM_FIRE_EVENT_E g_fire_cb_last_event = WUKONG_TM_FIRE_ALARM;
char g_fire_cb_last_id[WUKONG_TM_ALARM_ID_LEN + 1] = {0};

void test_fire_observer(const WUKONG_TM_FIRE_INFO_T *info)
{
    g_fire_cb_count++;
    g_fire_cb_last_event = info->event;
    strncpy(g_fire_cb_last_id, info->id, sizeof(g_fire_cb_last_id) - 1);
}
...
int main(void)
{
    ...
    alarm_cfg.repeat_type = WUKONG_TM_REPEAT_DAILY;
    alarm_cfg.hour = 7;
    alarm_cfg.minute = 30;
    ...
    EXPECT_OK(wukong_tm_fire_observer_set(test_fire_observer),
              "fire observer set should succeed");
    ...
    EXPECT(g_fire_cb_count >= 1, "fire observer invoked on alarm fire");
    EXPECT(g_fire_cb_last_event == WUKONG_TM_FIRE_ALARM,
           "fire event type is ALARM");
    EXPECT_STR_EQ(g_fire_cb_last_id, alarm_id,
                  "fire info carries the alarm id");
    EXPECT_STR_CONTAINS(g_last_cron_job_json,
    ...
    /* --- remove all --- */
    alarm_id = "test-alarm-rmall1";
    EXPECT_OK(wukong_tm_alarm_add(&alarm_cfg, alarm_id),
              "first alarm add for remove_all should succeed");
    duplicate_alarm_id = "test-alarm-rmall2";
    EXPECT_OK(wukong_tm_alarm_add(&alarm_cfg, duplicate_alarm_id),
              "second alarm add for remove_all should succeed");

    removed_count = 0;
    EXPECT_OK(wukong_tm_alarm_remove_all(&removed_count),
              "remove_all should succeed");
    EXPECT_EQ(removed_count, 2,
              "remove_all should report two deleted alarms");
