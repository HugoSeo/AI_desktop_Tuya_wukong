...
typedef struct {
    ...
    BOOL_T paused;
    SYS_TIME_T start_ms;
    SYS_TIME_T paused_elapsed_ms;
} WUKONG_TM_STOPWATCH_CTX_T;

STATIC WUKONG_TM_STOPWATCH_CTX_T s_stopwatch_ctx;

/**
 * @brief Compute total elapsed milliseconds for the current session.
 *
 * @return Elapsed milliseconds, or 0 when inactive.
 */
STATIC SYS_TIME_T __stopwatch_elapsed_ms(VOID)
{
    if (!s_stopwatch_ctx.active) {
        return 0;
    }
    if (s_stopwatch_ctx.paused) {
        return s_stopwatch_ctx.paused_elapsed_ms;
    }
    return s_stopwatch_ctx.paused_elapsed_ms +
           (tal_system_get_millisecond() - s_stopwatch_ctx.start_ms);
}

/**
 * @brief Compute the legacy whole-second view of the stopwatch.
 *
 * @return Elapsed whole seconds, or 0 when inactive.
 */
STATIC TIME_T __stopwatch_elapsed_sec(VOID)
{
    return (TIME_T)(__stopwatch_elapsed_ms() / 1000u);
}
...
OPERATE_RET wukong_tm_stopwatch_start(VOID)
{
    ...
    s_stopwatch_ctx.start_ms = tal_system_get_millisecond();
    s_stopwatch_ctx.paused_elapsed_ms = 0;
...
OPERATE_RET wukong_tm_stopwatch_pause(VOID)
{
    ...
    s_stopwatch_ctx.paused_elapsed_ms +=
        (tal_system_get_millisecond() - s_stopwatch_ctx.start_ms);
...
OPERATE_RET wukong_tm_stopwatch_query(WUKONG_TM_STOPWATCH_STATE_T *state)
{
    ...
    state->elapsed_ms = __stopwatch_elapsed_ms();
    state->elapsed_sec = (TIME_T)(state->elapsed_ms / 1000u);
    return OPRT_OK;
}
