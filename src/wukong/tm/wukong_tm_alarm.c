...
/**
 * @brief Storage record name (historically the KV key) for persisted alarm data.
 */
#define WUKONG_TM_ALARM_KV_KEY  "wk_tm_alarms"
/**
 * @brief Seconds in one day, used to roll once-alarms to tomorrow.
 */
#define WUKONG_TM_ALARM_ONCE_DAY_SEC  86400U
...
typedef struct {
    ...
    /** In-memory alarm table, WUKONG_TM_ALARM_MAX_COUNT entries. Heap-allocated
     *  in wukong_tm_alarm_init() and freed in wukong_tm_alarm_deinit(), so it
     *  costs PSRAM rather than BSS; it exists for the whole lifetime of the
     *  module, exactly like the fixed array it replaces. */
    WUKONG_TM_ALARM_ITEM_T *alarms;
...
#define WUKONG_TM_ALARM_TABLE_BYTES \
    (sizeof(*s_alarm_ctx.alarms) * WUKONG_TM_ALARM_MAX_COUNT)
...
/* Size of the transient cron-job JSON buffer. Allocated per call at the two
 * sites that build a job rather than held in BSS — the buffer is dead as soon
 * as wukong_cron_job_add() returns (it takes CONST CHAR_T * and duplicates
 * whatever it keeps). */
...
STATIC OPERATE_RET __alarm_once_rollover_to_next_fire(WUKONG_TM_ALARM_CFG_T *alarm_cfg);
STATIC INT_T __alarm_is_expired(CONST WUKONG_TM_ALARM_ITEM_T *alarm, TIME_T now,
                                CONST POSIX_TM_S *local_now);
STATIC BOOL_T __alarm_disable_expired_once_if_needed(WUKONG_TM_ALARM_ITEM_T *alarm, TIME_T now,
                                                     CONST POSIX_TM_S *local_now);
 ...
 *        load-first closure below). Bare bool: the event callback and the
 *        catch-up trampoline both run on WORKQ_SYSTEM (single worker), and
 *        the save-path window it additionally covers is boot-instant only —
 *        see __alarm_store_save().
 */
STATIC BOOL_T s_alarm_boot_load_done = FALSE;
/**
 ...
 */
STATIC BOOL_T s_alarm_evt_subscribed = FALSE;
#endif
...
STATIC BOOL_T __alarm_is_all_weekdays(CONST WUKONG_TM_ALARM_CFG_T *alarm_cfg)
{
    ...
    return (alarm_cfg->repeat_type == WUKONG_TM_REPEAT_WEEKLY &&
                                     ...
                                     CONST WUKONG_TM_ALARM_CFG_T *match_cfg)
{
    ...
    if (stored_cfg->repeat_type == match_cfg->repeat_type) {
        if (stored_cfg->repeat_type == WUKONG_TM_REPEAT_WEEKLY) {
            return (stored_cfg->weekday_mask == match_cfg->weekday_mask) ? TRUE : FALSE;
        }
        if (stored_cfg->repeat_type == WUKONG_TM_REPEAT_MONTHLY) {
            return (stored_cfg->month_day == match_cfg->month_day) ? TRUE : FALSE;
        }
        return TRUE;
    }

    if ((stored_cfg->repeat_type == WUKONG_TM_REPEAT_DAILY && __alarm_is_all_weekdays(match_cfg)) ||
        (match_cfg->repeat_type == WUKONG_TM_REPEAT_DAILY && __alarm_is_all_weekdays(stored_cfg))) {
        return TRUE;
    }
...
STATIC OPERATE_RET __alarm_validate_cfg(CONST WUKONG_TM_ALARM_CFG_T *alarm_cfg)
{
    ...
    if (alarm_cfg->repeat_type == WUKONG_TM_REPEAT_WEEKLY &&
        alarm_cfg->weekday_mask == 0) {
        return OPRT_INVALID_PARM;
    }

    if (alarm_cfg->repeat_type == WUKONG_TM_REPEAT_MONTHLY &&
...
/**
 * @brief Roll a once-alarm to the next valid fire time (today or tomorrow).
 *
 * When the wall-clock time (hour:minute) is earlier than now, add 24 hours so
 * the alarm fires tomorrow. When it is still later today, keep today's slot.
 * A future absolute start_time is left unchanged.
 *
 * @param[in,out] alarm_cfg Once-alarm configuration.
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET __alarm_once_rollover_to_next_fire(WUKONG_TM_ALARM_CFG_T *alarm_cfg)
{
    ...
    if (alarm_cfg->start_time > now) {
        return OPRT_OK;
    }

    /* tal_time_mktime treats input as UTC; compensate for timezone offset */
    local_now_copy = local_now;
    tz_offset = tal_time_mktime(&local_now_copy) - now;

    trigger_tm = local_now;
    trigger_tm.tm_hour = (INT_T)alarm_cfg->hour;
    trigger_tm.tm_min = (INT_T)alarm_cfg->minute;
    trigger_tm.tm_sec = 0;
    trigger_ts = tal_time_mktime(&trigger_tm) - tz_offset;
    if (trigger_ts == 0) {
        return OPRT_COM_ERROR;
    }

    if (trigger_ts <= now) {
        if (trigger_ts > (TIME_T)(-1 - (TIME_T)WUKONG_TM_ALARM_ONCE_DAY_SEC)) {
            return OPRT_COM_ERROR;
        }
        trigger_ts += (TIME_T)WUKONG_TM_ALARM_ONCE_DAY_SEC;
    }

    alarm_cfg->start_time = trigger_ts;
    if (tal_time_get_local_time_custom(trigger_ts, &trigger_tm) == OPRT_OK) {
        alarm_cfg->hour = (UINT_T)trigger_tm.tm_hour;
        alarm_cfg->minute = (UINT_T)trigger_tm.tm_min;
    }
    return OPRT_OK;
}

/**
 * @brief Disable one expired once-alarm and tear down its cron/runtime state.
 *
 * @param[in,out] alarm      Alarm slot.
 * @param[in]     now        Current POSIX timestamp.
 * @param[in]     local_now  Current local broken-down time.
 * @return TRUE when the alarm was auto-disabled.
 */
STATIC BOOL_T __alarm_disable_expired_once_if_needed(WUKONG_TM_ALARM_ITEM_T *alarm, TIME_T now,
                                                     CONST POSIX_TM_S *local_now)
{
    ...
    if (!alarm->used || alarm->cfg.repeat_type != WUKONG_TM_REPEAT_ONCE ||
        !alarm->cfg.enabled) {
        return FALSE;
    }
    /* Skip alarms inside an active ring/snooze cycle; let the runtime snooze
     * handlers finish them, otherwise a list query would tear down the pending
     * snooze job and silently cancel the replay. */
    if (alarm->is_ringing || alarm->ring_seq > 0 || alarm->snooze_job_id[0] != '\0') {
        return FALSE;
    }
    if (!__alarm_is_expired(alarm, now, local_now)) {
        return FALSE;
    }
    ...
    if (alarm->cfg.cron_job_id[0] != '\0') {
        (VOID)__alarm_remove_job_if_needed(alarm->cfg.cron_job_id);
    }
    TAL_PR_DEBUG("alarm -> auto-disabled expired once-alarm %s", alarm->alarm_id);
    return TRUE;
}
                                                ...
                                                CHAR_T *job_id, UINT_T job_id_len)
{
    ...
    cron_buf = tal_malloc(ALARM_CRON_BUF_SIZE);
    if (cron_buf == NULL) {
        TAL_PR_ERR("alarm -> cron job buffer alloc %d bytes failed", (INT_T)ALARM_CRON_BUF_SIZE);
        return OPRT_MALLOC_FAILED;
    }

    (VOID)snprintf(cron_buf, ALARM_CRON_BUF_SIZE,
    ...
    rt = wukong_cron_job_add(cron_buf, job_id, job_id_len);
    tal_free(cron_buf);
    return rt;
}
...
STATIC OPERATE_RET __alarm_sync_cron_job(WUKONG_TM_ALARM_ITEM_T *alarm)
{
    ...
    (VOID)snprintf(cron_buf, ALARM_CRON_BUF_SIZE,
                   ...
                   (alarm->cfg.repeat_type == WUKONG_TM_REPEAT_ONCE) ? 1 : 0, cron_expr,
                   alarm->alarm_id, WUKONG_TM_ALARM_FIRE_METHOD, alarm->alarm_id, escaped_message);

    rt = wukong_cron_job_add(cron_buf, cron_job_id, sizeof(cron_job_id));
    tal_free(cron_buf);
    if (rt != OPRT_OK) {
        return rt;
    }
...
STATIC OPERATE_RET __alarm_store_notify(VOID)
{
    UINT_T offset = 0;

    wukong_tm_changed_notify(WUKONG_TM_TYPE_ALARM);

    UINT8_T *msg = NULL;
    UINT_T len = 0;
    WUKONG_TM_TIMER_OPR_E opr = WUKONG_TM_TIMER_OPR_SYNC_LIST;
    WUKONG_TM_ALARM_ITEM_T *alarm_ptr = &s_alarm_ctx.alarms[0];
    UINT8_T alarm_count = (UINT8_T)WUKONG_TM_ALARM_MAX_COUNT;

    len += (WUKONG_TM_TLV_TL_LEN + 1);                      //opr
    len += (WUKONG_TM_TLV_TL_LEN + sizeof(alarm_ptr));      //alarm item pointer
    len += (WUKONG_TM_TLV_TL_LEN + 1);                      //alarm count
    msg = tal_malloc(len);
    if (msg == NULL) {
    ...
    __tm_tlv_pack(msg, WUKONG_TM_TAG_ALARM_ITEM, sizeof(alarm_ptr), (CONST UINT8_T *)&alarm_ptr, &offset);
    __tm_tlv_pack(msg, WUKONG_TM_TAG_ALARM_COUNT, 1, (CONST UINT8_T *)&alarm_count, &offset);
 ...
 * --------------------------------------------------------------------------- */
/**
 * @brief Serialize all in-memory alarms to persistent storage.
 *
 * @return OPRT_OK on success.
 */
STATIC OPERATE_RET __alarm_store_save(VOID)
{
...
#if WUKONG_TM_STORE_LOAD_DEFERRED
    /* Close the boot-time overwrite window: alarms persist as ONE JSON blob,
     * so a save issued after the volume mounted but before the deferred boot
     * load ran would clobber every not-yet-loaded alarm. Run the load first
     * (it merges into free slots and skips ids already in memory), then
    ...
    if (s_alarm_boot_load_done == FALSE) {
        /* set BEFORE loading: the load itself re-enters save when it disables
         * expired once-alarms. */
        s_alarm_boot_load_done = TRUE;
    ...
    rt = wukong_storage_write(WUKONG_TM_STORE_NS, WUKONG_TM_ALARM_KV_KEY,
                            (CONST BYTE_T *)json_str, strlen(json_str));
    ty_cJSON_FreeBuffer(json_str);
    if (rt != OPRT_OK) {
        TAL_PR_ERR("alarm -> store save failed, rt=%d", rt);
    }
    else {          //notify UI to sync alarm info!
        __alarm_store_notify();
    }
    return rt;
}

/**
 * @brief Load persisted alarms from storage into the in-memory table.
 *
...
STATIC OPERATE_RET __alarm_store_load(VOID)
{
    ...
    rt = wukong_storage_read(WUKONG_TM_STORE_NS, WUKONG_TM_ALARM_KV_KEY,
                           &data, &data_len);
    if (rt != OPRT_OK) {
    ...
    for (i = 0; i < count; i++) {
         ...
         * the deferred-load flows rely on (an alarm added before the boot
         * load must not be duplicated by its stored twin). */
        if (__alarm_find_index(node->valuestring) >= 0) {
            TAL_PR_NOTICE("alarm -> stored alarm %s already in memory, skipped", node->valuestring);
            continue;
        }
        ...
        alarm->cfg.repeat_type = (node != NULL && ty_cJSON_IsNumber(node))
                                 ? (WUKONG_TM_REPEAT_TYPE_E)node->valueint
                                 : WUKONG_TM_REPEAT_ONCE;
        ...
        expired = (now > 0 &&
                   alarm->cfg.repeat_type == WUKONG_TM_REPEAT_ONCE &&
    ...
    ty_cJSON_Delete(root);
    TAL_PR_NOTICE("alarm -> loaded %d alarms from storage", loaded);
    if (need_save) {
...
STATIC OPERATE_RET __alarm_start_ringing(WUKONG_TM_ALARM_ITEM_T *alarm)
{
    ...
    wukong_tm_fire_notify(WUKONG_TM_FIRE_ALARM, alarm->alarm_id, alarm->cfg.message);
    return OPRT_OK;
}
...
STATIC OPERATE_RET __alarm_snooze_timeout_rpc_handler(CONST ty_cJSON *params, ty_cJSON **result)
{
    ...
    if (index >= 0 && s_alarm_ctx.alarms[index].used &&
        ...
        if (s_alarm_snooze_enabled &&
            ...
            if (s_alarm_ctx.alarms[index].cfg.repeat_type == WUKONG_TM_REPEAT_ONCE) {
 ...
 *        WORKQ_SYSTEM (the event publishes from the mount job there; the
 *        catch-up path below trampolines onto the same worker), which
 *        serializes it against the catch-up — the bare bool is a sufficient
 *        idempotency guard (same convention as wukong_picture.c).
 *
 *        The tm boot load only needs the mounted volume — the storage record
 *        layer mkdirs its own namespace directory — so no ordering against
 *        other storage.ready consumers is required.
 */
STATIC OPERATE_RET __alarm_on_storage_ready(VOID_T *data)
{
    (VOID)data;
    if (!s_alarm_ctx.initialized || s_alarm_boot_load_done) {
        return OPRT_OK;
    }
    s_alarm_boot_load_done = TRUE;
    (VOID)__alarm_store_load();
 ...
 *        running inline on whatever thread called wukong_tm_alarm_init().
 */
STATIC VOID_T __alarm_catchup_on_workq(VOID_T *data)
{
    (VOID)data;
    (VOID)__alarm_on_storage_ready(NULL);
}
...
OPERATE_RET wukong_tm_alarm_init(VOID)
{
    ...
    s_alarm_ctx.alarms = tal_malloc(WUKONG_TM_ALARM_TABLE_BYTES);
    if (s_alarm_ctx.alarms == NULL) {
        TAL_PR_ERR("alarm -> table alloc %d bytes failed", (INT_T)WUKONG_TM_ALARM_TABLE_BYTES);
        return OPRT_MALLOC_FAILED;
    }
    memset(s_alarm_ctx.alarms, 0, WUKONG_TM_ALARM_TABLE_BYTES);
     ...
     * wukong_picture.c). */
    if (!s_alarm_evt_subscribed) {
        if (ty_subscribe_event(EVENT_WUKONG_STORAGE_READY, "wk_alarm",
                               __alarm_on_storage_ready, SUBSCRIBE_TYPE_NORMAL) != OPRT_OK) {
            TAL_PR_ERR("alarm -> subscribe storage.ready failed; stored alarms will not load");
        } else {
            s_alarm_evt_subscribed = TRUE;
        }
    }
    if (s_alarm_evt_subscribed && wukong_storage_ready()) {
        if (tal_workq_schedule(WORKQ_SYSTEM, __alarm_catchup_on_workq, NULL) != OPRT_OK) {
            TAL_PR_WARN("alarm -> catch-up schedule failed, stored alarms may not load");
        }
    }
#else
...
OPERATE_RET wukong_tm_alarm_deinit(VOID)
{
    ...
    /* Mandatory now that `alarms` is a heap pointer: it is NULL both when init
     * bailed out on a failed allocation and on a second deinit (the memset
     * below zeroes it), and the walk would dereference it. The sibling modules
     * (wukong_cron_deinit / wukong_tm_reminder_deinit) already guard this way. */
    if (!s_alarm_ctx.initialized) {
        return OPRT_OK;
    }
    ...
    tal_free(s_alarm_ctx.alarms);
    s_alarm_ctx.alarms = NULL;
    memset(&s_alarm_ctx, 0, sizeof(s_alarm_ctx));
#if WUKONG_TM_STORE_LOAD_DEFERRED
    /* The context is now empty, so the persisted alarms must be re-read on the
     * next init — clear the one-shot latch or a reinit without a reboot would
     * skip the load and the first save would write the empty table back over
     * them. s_alarm_evt_subscribed stays sticky on purpose (base_event has no
     * unsubscribe here); init's catch-up only needs it TRUE to reschedule. */
    s_alarm_boot_load_done = FALSE;
#endif
    return OPRT_OK;
}

/**
 * @brief Delete persisted alarm data from persistent storage.
 *
 * Call after wukong_tm_alarm_deinit() during a device reset so that
 * expired once-alarms do not survive across resets and occupy all slots.
 *
 * @return OPRT_OK on success.
 */
OPERATE_RET wukong_tm_alarm_kv_clear(VOID)
{
    return wukong_storage_delete(WUKONG_TM_STORE_NS, WUKONG_TM_ALARM_KV_KEY);
}
 ...
 * @note Enabling an expired once-alarm rolls its trigger to the next valid
 *       slot (today if still upcoming, otherwise +24 hours).
 */
OPERATE_RET wukong_tm_alarm_enable_set(CONST CHAR_T *alarm_id, BOOL_T enable)
{
    ...
    if (!enable) {
        ...
        if (alarm->cfg.repeat_type == WUKONG_TM_REPEAT_ONCE) {
            if (__alarm_once_rollover_to_next_fire(&alarm->cfg) != OPRT_OK) {
                alarm->cfg.enabled = FALSE;
                return OPRT_COM_ERROR;
            }
            TAL_PR_NOTICE("alarm -> enable once-alarm %s, start_time=%d",
                          alarm->alarm_id, (INT_T)alarm->cfg.start_time);
        }
...
OPERATE_RET wukong_tm_alarm_add(CONST WUKONG_TM_ALARM_CFG_T *alarm_cfg, CONST CHAR_T *alarm_id)
{
    ...
    if (alarm_cfg->enabled && alarm_cfg->repeat_type == WUKONG_TM_REPEAT_ONCE) {
        if (__alarm_once_rollover_to_next_fire(&s_alarm_ctx.alarms[index].cfg) != OPRT_OK) {
            memset(&s_alarm_ctx.alarms[index], 0, sizeof(s_alarm_ctx.alarms[index]));
            return OPRT_COM_ERROR;
        }
    }
...
OPERATE_RET wukong_tm_alarm_update(CONST CHAR_T *alarm_id, CONST WUKONG_TM_ALARM_CFG_T *alarm_cfg)
{
    ...
    if (s_alarm_ctx.alarms[index].cfg.enabled &&
        s_alarm_ctx.alarms[index].cfg.repeat_type == WUKONG_TM_REPEAT_ONCE) {
        if (__alarm_once_rollover_to_next_fire(&s_alarm_ctx.alarms[index].cfg) != OPRT_OK) {
 ...
 *
 * @param[out] removed_count   Optional number of removed alarms.
 * @return OPRT_OK on success (even if no alarms existed).
 */
OPERATE_RET wukong_tm_alarm_remove_all(UINT_T *removed_count)
{
    UINT_T removed = 0;
    INT_T index = 0;
    OPERATE_RET rt = OPRT_OK;
...
OPERATE_RET wukong_tm_alarm_list(CHAR_T **alarm_list_json)
{
    ...
    *alarm_list_json = NULL;
    ...
    if (has_local_time) {
        for (index = 0; index < WUKONG_TM_ALARM_MAX_COUNT; index++) {
            if (__alarm_disable_expired_once_if_needed(&s_alarm_ctx.alarms[index], now, &local_now)) {
                need_save = TRUE;
            }
        }
        if (need_save) {
    ...
    if (has_local_time) {
        CHAR_T device_time_iso[32] = {0};
        wukong_iso8601_format(tal_time_get_posix(), device_time_iso, sizeof(device_time_iso));
        ty_cJSON_AddStringToObject(root, "device_time", device_time_iso);
    }
    ...
    for (index = 0; index < WUKONG_TM_ALARM_MAX_COUNT; index++) {
        ...
        if (alarm->cfg.repeat_type == WUKONG_TM_REPEAT_ONCE &&
            alarm->cfg.start_time > 0) {
            CHAR_T alarm_time_iso[32] = {0};
            wukong_iso8601_format(alarm->cfg.start_time, alarm_time_iso, sizeof(alarm_time_iso));
            (VOID)snprintf(time_str, sizeof(time_str), "%s", alarm_time_iso);
        } else {
 ...
 *
 * For ONCE alarms the absolute trigger timestamp (`start_time`) is part of the
 * match key, so two one-time alarms at the same wall-clock hour:minute on
 * different dates are correctly treated as distinct. Recurring alarms keep
 * the previous semantics (hour/minute + repeat shape).
 *
                                         ...
                                         CHAR_T *alarm_id, UINT_T alarm_id_len)
{
    ...
    for (index = 0; index < WUKONG_TM_ALARM_MAX_COUNT; index++) {
        ...
        /* For ONCE alarms hour:minute alone is not unique: two one-time
         * alarms at the same wall-clock minute on different dates must NOT
         * be reported as duplicates. Compare absolute timestamps when both
         * sides have one set; if either side is unset (legacy data),
         * fall back to the hour:minute match so callers without timestamp
         * context (e.g. cleanup helpers) keep working. */
        if (alarm_cfg->repeat_type == WUKONG_TM_REPEAT_ONCE &&
            alarm->cfg.start_time > 0 && alarm_cfg->start_time > 0 &&
            alarm->cfg.start_time != alarm_cfg->start_time) {
            continue;
        }
