...
#include "tal_system.h"   /* SYS_TIME_T (stopwatch elapsed_ms) */
 ...
 */
typedef enum {
    WUKONG_TM_REPEAT_ONCE = 0,     /**< Fire only once. */
    WUKONG_TM_REPEAT_DAILY = 1,    /**< Every day at hour:minute. */
    WUKONG_TM_REPEAT_WEEKLY = 2,   /**< Specific weekdays at hour:minute. */
    WUKONG_TM_REPEAT_MONTHLY = 3,  /**< Specific day-of-month at hour:minute. */
} WUKONG_TM_REPEAT_TYPE_E;
 ...
 */
typedef struct {
    BOOL_T enabled;                                           /**< Whether the reminder is enabled. */
    WUKONG_TM_REPEAT_TYPE_E repeat_type;                     /**< Recurrence type (once, daily, weekly, monthly). */
    UINT_T hour;                                              /**< Trigger hour [0, 23] (used for recurring). */
    UINT_T minute;                                            /**< Trigger minute [0, 59] (used for recurring). */
    UINT_T weekday_mask;                                      /**< Weekly bitmask bit0=Sun..bit6=Sat (for weekly). */
    UINT_T month_day;                                         /**< Day of month [1, 31] (for monthly). */
    TIME_T start_time;                                        /**< Absolute trigger time (UTC POSIX); for recurring, anchor/first fire. */
    CHAR_T message[WUKONG_TM_REMINDER_MESSAGE_LEN + 1];     /**< Reminder message text spoken at fire time. */
    CHAR_T cron_job_id[WUKONG_TM_REMINDER_CRON_JOB_ID_LEN + 1]; /**< Bound cron job id. */
...
typedef struct {
    ...
    BOOL_T paused;
    /** Total elapsed milliseconds since start, excluding paused intervals. */
    SYS_TIME_T elapsed_ms;
    /** Compatibility view of elapsed_ms rounded down to whole seconds. */
    TIME_T elapsed_sec;
} WUKONG_TM_STOPWATCH_STATE_T;

/**
 * @brief Time-manage fire event categories surfaced to observers (e.g. UI).
 */
typedef enum {
    WUKONG_TM_FIRE_ALARM = 0,        /**< Alarm entered its ringing window. */
    WUKONG_TM_FIRE_REMINDER,         /**< Reminder reached its trigger time. */
    WUKONG_TM_FIRE_COUNTDOWN_DONE,   /**< Countdown reached zero. */
    WUKONG_TM_FIRE_POMODORO_PHASE,   /**< Pomodoro switched phase. */
} WUKONG_TM_FIRE_EVENT_E;

/**
 * @brief Payload delivered to a fire observer.
 */
typedef struct {
    WUKONG_TM_FIRE_EVENT_E event;
    CHAR_T id[WUKONG_TM_ALARM_ID_LEN + 1];
    CHAR_T message[WUKONG_TM_ALARM_MESSAGE_LEN + 1];
} WUKONG_TM_FIRE_INFO_T;

/**
 * @brief Fire observer callback type. Invoked synchronously from the firing
 *        thread (cron/business). Observers MUST NOT block and MUST marshal any
 *        UI work onto the UI thread themselves.
 */
typedef VOID_T (*WUKONG_TM_FIRE_CB)(CONST WUKONG_TM_FIRE_INFO_T *info);

/**
 * @brief Register the single fire observer (last writer wins; NULL clears).
 */
OPERATE_RET wukong_tm_fire_observer_set(WUKONG_TM_FIRE_CB cb);

/**
 * @brief List-change observer callback type.
 *
 * Invoked whenever the persisted alarm or reminder set changes (add, update,
 * remove, enable/disable), regardless of the trigger source (voice/AI/cloud/MCP
 * or local UI). Called synchronously from the business thread inside the
 * store-save path; observers MUST NOT block and MUST marshal any UI work onto
 * the UI thread themselves.
 *
 * @param[in] type The feature whose list changed (alarm or reminder).
 */
typedef VOID_T (*WUKONG_TM_CHANGE_CB)(WUKONG_TM_TYPE_E type);

/**
 * @brief Register the single list-change observer (last writer wins; NULL clears).
 */
