...
STATIC BOOL_T __s_tts_ignore_stale_stop = FALSE;
STATIC BOOL_T __s_tts_wait_player_stop = FALSE;
...
STATIC OPERATE_RET __audio_output_open(UINT32_T sample_rate, UINT8_T sample_bits, UINT8_T channel)
{
    ...
    cfg.channel = channel;
    cfg.pa_gpio = TUYA_AI_TOY_SPK_EN_PIN;
    cfg.pa_active_level = TUYA_GPIO_LEVEL_HIGH;
    cfg.frame_time_ms = 20;
...
OPERATE_RET __player_event(VOID *data)
{
    ...
    tuya_ai_player_get_volume(__s_tone_player, &player_vol);
    //! player finished or failed.
    if (event->state == AI_PLAYER_STOPPED) {
        TAL_PR_DEBUG("wukong audio player -> stop event");
        if (event->handle == __s_tone_player && __s_tts_play_flag && __s_tts_ignore_stale_stop) {
            TAL_PR_DEBUG("wukong audio player -> ignore stale tts stop event");
            __s_tts_ignore_stale_stop = FALSE;
            __s_tts_wait_player_stop = FALSE;
            return rt;
        }
        if (event->handle == __s_tone_player) {
            __s_tts_wait_player_stop = FALSE;
        }
        ...
        if (!wukong_audio_player_is_playing()) {
            ...
            if (__s_music_player) tuya_ai_player_set_volume(__s_music_player, player_vol);
        }
    }
    else if (event->state == AI_PLAYER_PLAYING) {
        TAL_PR_DEBUG("wukong audio player -> playing start event");
        if (event->handle == __s_tone_player && __s_music_player && AI_PLAYER_PLAYING == tuya_ai_player_get_state(__s_music_player)) {
    ...
    if(event->handle == __s_music_player)
    {
        ...
        music_msg.reason = event->reason;
        TAL_PR_DEBUG("[%s] cmd: %d, state: %d, reason: %d", __FUNCTION__, music_msg.cmd, music_msg.state, music_msg.reason);
...
OPERATE_RET wukong_audio_player_init(VOID)
{
    ...
    // player init
    AI_PLAYER_CFG_T cfg = {.sample_rate = 16000, .sample_bits = 16, .channel = 1};
...
BOOL_T wukong_audio_player_is_playing(VOID)
{
    if (tuya_ai_player_get_state(__s_tone_player) == AI_PLAYER_PLAYING ||
        (__s_music_player && (tuya_ai_player_get_state(__s_music_player) == AI_PLAYER_PLAYING))) {
            return TRUE;
    } 
...
OPERATE_RET wukong_audio_play_tts_stream(WUKONG_AI_EVENT_TYPE_E type, AI_AUDIO_CODEC_E codec, CHAR_T *data, INT_T len)
{
    ...
    {
        AI_PLAYER_STATE_T tts_state = tuya_ai_player_get_state(__s_tone_player);
        TAL_PR_DEBUG("wukong audio player -> tts stream start");
        __s_tts_play_flag = TRUE;
        __s_tts_ignore_stale_stop = (tts_state != AI_PLAYER_STOPPED || __s_tts_wait_player_stop);
        if (__s_tts_ignore_stale_stop) {
            TAL_PR_DEBUG("wukong audio player -> wait stale tts stop event");
        }
    ...
    case WUKONG_AI_EVENT_TTS_STOP:
        ...
        __s_tts_wait_player_stop = (tuya_ai_player_get_state(__s_tone_player) != AI_PLAYER_STOPPED);
        TUYA_CALL_ERR_LOG(tuya_ai_player_feed(__s_tone_player, NULL, 0));
...
{
    tuya_ai_player_set_volume(__s_tone_player, vol);

    if (__s_music_player) {
        tuya_ai_player_set_volume(__s_music_player, __s_tts_play_flag ? vol/2 : vol);
    }
    return OPRT_OK;
}

/**
 * @brief Get current music playback progress (byte offset / total length)
 * @param[out] offset  Current byte offset into the audio source
 * @param[out] length  Total length in bytes (0 if unknown, e.g. live network stream)
 * @return OPRT_OK on success; OPRT_COM_ERROR if no music player is active;
 *         OPRT_INVALID_PARM if offset/length is NULL
 */
OPERATE_RET wukong_audio_player_get_progress(UINT_T *offset, UINT_T *length)
{
    ...
    *offset = 0;
    *length = 0;
    if (__s_music_player == NULL) {
        return OPRT_COM_ERROR;
    }
    return tuya_ai_player_get_progress(__s_music_player, offset, length);
}
