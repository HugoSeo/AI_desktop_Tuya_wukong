...
typedef struct {
    ...
    UINT8_T                     channel;           // processed audio channel count
...
typedef struct {
     ...
     *  @return OPRT_OK on success */
    OPERATE_RET (*set_vol)(UINT8_T volume);
...
/** @brief Get the last wake-up state set via wukong_audio_input_wakeup_set
 *  @return TRUE if wake-up currently enabled, FALSE otherwise
 *  @note  Reflects the value cached at this wrapper layer; valid in manual VAD
 *         mode where wake-up only changes through explicit set calls. */
...
/** @brief Set audio input volume/gain
 *  @param[in] volume  Volume level 0-100
 *  @return OPRT_OK on success */
