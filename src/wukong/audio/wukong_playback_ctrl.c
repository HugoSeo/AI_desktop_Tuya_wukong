...
#include "wukong_storage.h"    /* WUKONG_STORAGE_ENABLE, EVENT_WUKONG_STORAGE_READY, wukong_storage_ready() */
#include "tuya_app_config.h"

/* Compile-time gate: defer the one-shot boot playlist load until the external
 * storage volume is mounted (EVENT_WUKONG_STORAGE_READY). The playlist
 * persists through the storage record layer (wukong_storage.h), whose only
 * medium is the external storage volume: when a volume exists
 * (WUKONG_STORAGE_ENABLE) it mounts asynchronously, so the boot load must
 * wait for storage.ready. On NONE boards (and host unit-test builds, which
 * define no storage macros) there is no medium at all — the event is never
 * published and nothing persists — so the load is NOT deferred: deferring
 * would mean never loading, while the immediate load harmlessly reads back
 * an empty table. */
#if defined(WUKONG_STORAGE_ENABLE) && (WUKONG_STORAGE_ENABLE == 1)
#define PLAYBACK_STORAGE_LOAD_DEFERRED 1
#include "tal_workq_service.h"    /* WORKQ_SYSTEM catch-up trampoline */
#else
#define PLAYBACK_STORAGE_LOAD_DEFERRED 0
#endif
...
#if PLAYBACK_STORAGE_LOAD_DEFERRED
/* Boot playlist load armed but not yet run (waiting for storage.ready).
 * Read/cleared under s_playback_mutex: unlike the bare-bool consumers that
 * only touch their flag on WORKQ_SYSTEM, this one is also cleared from the
 * persist path (any caller thread) — see __playback_persist_and_notify(). */
STATIC BOOL_T s_storage_load_pending = FALSE;
#endif
...
/* Playback mode (in-memory; default sequence = legacy behaviour) */
STATIC WK_PLAY_MODE_E s_play_mode = WK_PLAY_MODE_SEQUENCE;
/* Simple LCG state for shuffle (no SDK RNG dependency needed) */
STATIC UINT_T s_shuffle_rng = 0x1234567u;
...
/* User-triggered prev: local navigation executed on the worker thread */
STATIC BOOL_T s_user_prev_pending = FALSE;
...
#if PLAYBACK_STORAGE_LOAD_DEFERRED
    /* Close the boot-time overwrite window: a playlist mutation that lands
     * after the volume mounted but before the deferred boot load ran means
     * the in-memory list is now the newer truth (e.g. a fresh cloud playlist
     * push) — cancel the pending load so it cannot clobber memory back with
     * the stale on-disk list, and persist as usual. If the volume is NOT
     * mounted yet, keep the load armed: the save below fails harmlessly (the
     * storage record layer refuses unmounted writes, disk data intact) and
     * the persisted playlist still loads once storage comes up. */
    if (wukong_storage_ready()) {
        tal_mutex_lock(s_playback_mutex);
        if (s_storage_load_pending == TRUE) {
            s_storage_load_pending = FALSE;
            TAL_PR_NOTICE("[playback] persist before deferred boot load: in-memory list wins, boot load canceled");
        }
...
#if PLAYBACK_STORAGE_LOAD_DEFERRED
/**
 * @brief Deferred boot playlist load, run on WORKQ_SYSTEM once storage is up
 *        (base_event callback path and the register_storage catch-up path
 *        both land here). Idempotency + newer-truth arbitration are carried
 *        by s_storage_load_pending under the playback mutex: the persist path
 *        cancels a still-pending load when a mutation beats it (see
 *        __playback_persist_and_notify()).
 */
STATIC OPERATE_RET __playback_on_storage_ready(VOID_T *data)
{
    BOOL_T do_load = FALSE;
    ...
    TAL_PR_NOTICE("[playback] storage.ready: running deferred playlist load");
    __playback_storage_load_from_ops();
    if (s_storage_ops != NULL && s_storage_ops->on_changed != NULL) {
        s_storage_ops->on_changed();
    }
    return OPRT_OK;
}
 ...
 *        running inline on whatever thread registered the storage ops.
 */
STATIC VOID_T __playback_catchup_on_workq(VOID_T *data)
{
    (VOID)data;
    (VOID)__playback_on_storage_ready(NULL);
}
#endif /* PLAYBACK_STORAGE_LOAD_DEFERRED */
...
OPERATE_RET wukong_playback_ctrl_register_storage(CONST WUKONG_PLAYBACK_STORAGE_OPS_T *ops)
{
    ...
    /* Swap to the new storage. s_storage_ops is a borrowed const pointer (the
     * ops struct is owned by the caller, normally static), so there is nothing
     * to free for the previous registration. */
    s_storage_ops = ops;
    ...
    /* Late (re-)registration: reload the playlist from the new storage so
     * persisted data is picked up. __playback_storage_load_from_ops clears the
     * in-memory list then applies the loaded JSON under the lock, so this is a
     * clean swap. Intended for the one-time boot registration; not designed to
     * run mid-playback. */
    if (s_playback_inited == TRUE) {
#if PLAYBACK_STORAGE_LOAD_DEFERRED
        /* FS-backed playlist: the volume mounts asynchronously on
         * WORKQ_SYSTEM. Loading right now would read NOT_FOUND, leave the
         * playlist empty for the whole boot, and any later save would then
         * overwrite playlist.json with that empty/new list (real data loss).
         * Defer the one-shot load: subscribe first, then catch up if storage
         * won the race (canonical consumer pattern, see ui_svc_fs.c). */
        STATIC BOOL_T s_storage_evt_subscribed = FALSE;

        tal_mutex_lock(s_playback_mutex);
        s_storage_load_pending = TRUE;
        tal_mutex_unlock(s_playback_mutex);
        TAL_PR_NOTICE("[playback] register_storage after init: load deferred until storage.ready");
        if (!s_storage_evt_subscribed) {
            if (ty_subscribe_event(EVENT_WUKONG_STORAGE_READY, "playback",
                                   __playback_on_storage_ready, SUBSCRIBE_TYPE_NORMAL) != OPRT_OK) {
                TAL_PR_ERR("[playback] subscribe storage.ready failed; playlist will not load");
            } else {
                s_storage_evt_subscribed = TRUE;
            }
        }
        if (s_storage_evt_subscribed && wukong_storage_ready()) {
            if (tal_workq_schedule(WORKQ_SYSTEM, __playback_catchup_on_workq, NULL) != OPRT_OK) {
                TAL_PR_WARN("[playback] catch-up schedule failed, playlist may not load");
            }
        }
#else
        /* Non-FS (KV/UF/CUSTOM) backends have no asynchronous mount to wait
         * for — load immediately, as before. */
        TAL_PR_NOTICE("[playback] register_storage after init: reload from new storage");
        __playback_storage_load_from_ops();
        if (s_storage_ops->on_changed != NULL) {
            s_storage_ops->on_changed();
        }
...
OPERATE_RET wukong_playback_ctrl_fetch_more(VOID)
{
    ...
    tal_mutex_lock(s_playback_mutex);
    if (!s_search_ctx.valid) {
        /* No prior search (cold boot): fall back to a default filter-less
         * browse so UI next/autoplay can still pull songs from the cloud.
         * Pagination then continues from this ctx like a normal search. */
        memset(&s_search_ctx, 0, sizeof(s_search_ctx));
        s_search_ctx.limit       = 5;
        s_search_ctx.next_offset = 0;
        s_search_ctx.has_more    = TRUE;
        s_search_ctx.valid       = TRUE;
        TAL_PR_NOTICE("[playback] fetch_more: no search ctx, default browse");
    } else if (!s_search_ctx.has_more) {
        tal_mutex_unlock(s_playback_mutex);
        TAL_PR_DEBUG("[playback] fetch_more: no more pages");
        return OPRT_COM_ERROR;
    }
...
{
    if (mode >= WK_PLAY_MODE_MAX) {
        return;
    }
    tal_mutex_lock(s_playback_mutex);
    s_play_mode = mode;
    tal_mutex_unlock(s_playback_mutex);
    TAL_PR_NOTICE("[playback] play mode -> %d", mode);

    /* Notify UI so the mode label refreshes when changed via MCP/skill. */
    WUKONG_MUSIC_PLAYER_T ui_msg = {0};
    ui_msg.cmd = MUSIC_PLAYER_STATE;
    tal_mutex_lock(s_playback_mutex);
    ui_msg.state = s_music_player.state;
    tal_mutex_unlock(s_playback_mutex);
...
/* caller must hold s_playback_mutex */
STATIC INT_T __playback_playlist_first_id_locked(VOID)
{
    LIST_HEAD *pos = NULL;
    tuya_list_for_each(pos, &s_playlist.list_head) {
        WUKONG_PLAYBACK_ITEM_T *item = tuya_list_entry(pos, WUKONG_PLAYBACK_ITEM_T, list_node);
...
/* caller must hold s_playback_mutex */
STATIC INT_T __playback_playlist_nth_id_locked(INT_T n)
{
    LIST_HEAD *pos = NULL;
    INT_T i = 0;
    tuya_list_for_each(pos, &s_playlist.list_head) {
...
/* caller must hold s_playback_mutex; returns a random id != avoid_id when possible */
STATIC INT_T __playback_playlist_random_id_locked(INT_T avoid_id)
{
    if (s_playlist.num <= 0) {
        return -1;
    }
    if (s_playlist.num == 1) {
        return __playback_playlist_first_id_locked();
    }
    s_shuffle_rng = s_shuffle_rng * 1103515245u + 12345u;
    INT_T n = (INT_T)((s_shuffle_rng >> 16) % (UINT_T)s_playlist.num);
    INT_T id = __playback_playlist_nth_id_locked(n);
    if (id == avoid_id) {
        n = (n + 1) % s_playlist.num;
        id = __playback_playlist_nth_id_locked(n);
    }
    return id;
}
...
STATIC VOID __auto_next_thread_func(VOID *arg)
{
    ...
    for (;;) {
        ...
        BOOL_T user_prev = s_user_prev_pending;
        INT_T  user_play = s_user_play_id;
        s_user_next_pending = FALSE;
        s_user_prev_pending = FALSE;
        s_user_play_id      = -1;
        tal_mutex_unlock(s_playback_mutex);
        ...
        /* User-requested previous — local navigation only, no settle delay.
         * At the beginning of the list the request is dropped silently. */
        if (user_prev) {
            if (wukong_playback_playlist_prev() != OPRT_OK) {
                TAL_PR_NOTICE("[%s] user-prev: no previous track", __func__);
            }
            continue;
        }
        ...
        WK_PLAY_MODE_E mode = s_play_mode;
        INT_T cur_id = s_current_play_id;
        INT_T single_id = (mode == WK_PLAY_MODE_SINGLE_LOOP) ? cur_id : -1;
        INT_T shuffle_id = (mode == WK_PLAY_MODE_SHUFFLE)
                               ? __playback_playlist_random_id_locked(cur_id) : -1;
        tal_mutex_unlock(s_playback_mutex);

        /* User-triggered next always advances sequentially regardless of mode. */
        if (!user_next && mode == WK_PLAY_MODE_SINGLE_LOOP && single_id >= 0) {
            TAL_PR_NOTICE("[%s] single-loop: replay id=%d", __func__, single_id);
            wukong_playback_playlist_play(single_id);
            continue;
        }
        if (!user_next && mode == WK_PLAY_MODE_SHUFFLE && shuffle_id >= 0) {
            TAL_PR_NOTICE("[%s] shuffle: play id=%d", __func__, shuffle_id);
            wukong_playback_playlist_play(shuffle_id);
            continue;
        }
        ...
        if (!user_next && mode == WK_PLAY_MODE_LIST_LOOP) {
            tal_mutex_lock(s_playback_mutex);
            INT_T first_id = __playback_playlist_first_id_locked();
            tal_mutex_unlock(s_playback_mutex);
            if (first_id >= 0) {
                TAL_PR_NOTICE("[%s] list-loop: wrap to first id=%d", __func__, first_id);
                wukong_playback_playlist_play(first_id);
                continue;
            }
        }
...
STATIC INT_T __music_player_event_cb(VOID *data)
{
    ...
    switch (cmd)
    {
        case MUSIC_PLAYER_STATE:
            ...
            TAL_PR_DEBUG("[playback] state -> %s (cur_id:%d switching:%d auto:%d reason:%d)",
                         (msg->state <= AI_PLAYER_PAUSED) ? state_names[msg->state] : "?",
                         s_current_play_id, s_switching_track, s_auto_play_next, msg->reason);
            if (msg->state == AI_PLAYER_PLAYING) {
                s_switching_track = FALSE;
            } else if (msg->state == AI_PLAYER_STOPPED) {
                /* Auto-advance only when the track actually ended (EOF) or failed
                 * mid-play (ERROR -> skip the broken track). Use an explicit
                 * whitelist so any other/future stop reason (user STOP cmd, mode
                 * switch, chat break, ...) defaults to NOT auto-advancing --
                 * otherwise e.g. double-pressing the mode key would skip a song. */
                if ((msg->reason == AI_PLAYER_STOP_EOF || msg->reason == AI_PLAYER_STOP_ERROR) &&
                    s_auto_play_next && !s_switching_track &&
...
OPERATE_RET wukong_playback_playlist_play(INT_T id)
{
    ...
    /* Set cur_id and metadata BEFORE starting playback so the audio engine's
     * async PLAYING callback reads the correct current_play_id. */
    tal_mutex_lock(s_playback_mutex);
    s_switching_track = TRUE;
 ...
 *        Fully asynchronous: navigation runs on the worker thread so the
 *        caller never blocks on URL refresh; at the end of the local playlist
 *        the worker falls back to fetch_more. Result is delivered via
 *        EVENT_MUSIC_PLAYER.
...
OPERATE_RET wukong_playback_ctrl_next(VOID)
{
    ...
    TAL_PR_NOTICE("[playback] user next scheduled");
    return OPRT_OK;
}

/**
 * @brief Non-blocking "prev" for UI callbacks.
 *        Dispatches wukong_playback_playlist_prev() to the worker thread so
 *        the caller (LVGL task) never blocks on URL refresh. At the beginning
 *        of the playlist the request is dropped (current track keeps playing).
 * @return OPRT_OK if queued; OPRT_COM_ERROR if module not ready.
 */
OPERATE_RET wukong_playback_ctrl_prev(VOID)
{
    ...
    tal_mutex_lock(s_playback_mutex);
    s_user_prev_pending = TRUE;
    tal_mutex_unlock(s_playback_mutex);

    tal_semaphore_post(s_auto_next_sem);

    TAL_PR_NOTICE("[playback] user prev scheduled");
    return OPRT_OK;
}
