...
class CTestHarness:
    ...
    def __init__(self, tmp_path: Path, tests_dir: Path) -> None:
        ...
        self._front_includes: list[Path] = []
    ...
    def include_sdk_front(self, rel_path: str) -> "CTestHarness":
        """Prepend an include path (relative to apps/.../src/) BEFORE
        tests_common/stubs, for suites needing a real SDK header over a
        same-named stub (e.g. wukong_ai_agent.h for the agent loop test)."""
        self._front_includes.append(_APP_SRC / rel_path)
        return self

    def define(self, macro: str) -> "CTestHarness":
        """Add a preprocessor define, e.g. "FOO=1" or "FOO" (-DFOO[=value]).

        Used to compile a KConfig-gated .c against a specific compile-time
        choice (host tests never run KConfig, see tuya_app_config.h stub), so
        a suite can exercise a real #if branch instead of only its default.
        """
        self._defines.append(macro)
        return self
    ...
    def run(self) -> None:
        ...
        cmd = [
            "cc",
            *self._CC_FLAGS,
            *[f"-I{d}" for d in self._front_includes],
            *[f"-I{d}" for d in self._includes],
            *[f"-D{d}" for d in self._defines],
