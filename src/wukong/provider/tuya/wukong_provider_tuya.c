...
STATIC OPERATE_RET __tuya_text_cb(AI_TEXT_TYPE_E type, ty_cJSON *root, BOOL_T eof)
{
    ...
    wukong_fc_process(type, root, eof);
    return OPRT_OK;
}
...
STATIC OPERATE_RET __tuya_alert_cb(AI_ALERT_TYPE_E type)
{
    if (type == AT_PLEASE_AGAIN) {
        ...
        wukong_ai_event_notify(WUKONG_AI_EVENT_ASR_EMPTY, NULL);
        return OPRT_OK;
    }
...
STATIC OPERATE_RET __tuya_init(VOID_T *handle, CONST WUKONG_AI_PROVIDER_CFG_T *cfg)
{
...
#endif
#if defined(APP_OPUS_ENCODER_VBR_ENABLE) && (APP_OPUS_ENCODER_VBR_ENABLE == 1)
    ai_agent_cfg.attr.audio.vbr = 1;
    /* DTX 依赖 VBR，仅在 VBR 开启时生效 */
#if defined(APP_OPUS_ENCODER_DTX_ENABLE) && (APP_OPUS_ENCODER_DTX_ENABLE == 1)
    ai_agent_cfg.attr.audio.dtx = 1;
#endif
#endif
    ai_agent_cfg.attr.audio.frame_duration = 40;  // Opus frame duration in ms (must be 10/20/40/60); frame_size is computed by svc_ai_basic from bitrate+frame_duration
    ...
    /* MCP inbound: the SDK delivers a parsed ty_cJSON straight to the MCP router.
     * wukong_mcp_recv is a zero-overhead alias to the router (same signature). */
    tuya_ai_agent_mcp_set_cb(wukong_mcp_recv, NULL);
#endif
...
STATIC OPERATE_RET __tuya_mcp_send(VOID_T *handle, CHAR_T *sid, CHAR_T *eid,
                                   CONST CHAR_T *message)
{
    (VOID_T)handle;
    return tuya_ai_agent_mcp_response(sid, eid, (CHAR_T *)message);
}
...
STATIC OPERATE_RET __tuya_abort(VOID_T *handle, CONST CHAR_T *chat_id)
{
    (VOID_T)handle;
    (VOID_T)chat_id;
    /* 设备端主动打断(唤醒词/按键/模式切换等)：云端不一定回发 CHAT_BREAK，
     * 这里同步复位 NLG 流状态，与云端打断分支保持一致 */
    wukong_fc_notify_chat_break();
...
STATIC OPERATE_RET __tuya_ioctl(VOID_T *handle, INT_T cmd, VOID_T *arg)
{
    ...
    case WUKONG_PROVIDER_CMD_DEL_SESSION:
        return tuya_ai_agent_del_session((CONST CHAR_T *)arg);
    default:
        return OPRT_NOT_SUPPORTED;
    }
}
