...
/* ─── Claw LLM Backend Format ─── */
typedef enum {
    WUKONG_LLM_BACKEND_OPENAI = 0,   /* OpenAI-compatible: /chat/completions, Bearer */
    WUKONG_LLM_BACKEND_ANTHROPIC,    /* Anthropic: /v1/messages, x-api-key (M1 未实现) */
} WUKONG_LLM_BACKEND_E;
...
typedef struct {
    ...
    WUKONG_LLM_BACKEND_E backend;  /* Claw 用,选 LLM API 格式;Cloud provider 忽略 */
...
typedef enum {
    ...
    WUKONG_PROVIDER_CMD_DEL_SESSION,
...
typedef struct {
    ...
    /* MCP outbound: emit one serialized JSON-RPC message over this provider's
     * connection (device acts as MCP server toward the cloud LLM).
     * NULL = this provider has no MCP channel. */
    OPERATE_RET (*mcp_send)(VOID_T *handle, CHAR_T *sid, CHAR_T *eid,
