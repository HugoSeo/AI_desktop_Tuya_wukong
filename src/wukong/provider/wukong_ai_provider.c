...
/* --- Default provider name --- */
  ...
  #if defined(ENABLE_PROVIDER_JD) && (ENABLE_PROVIDER_JD == 1)
    #define WUKONG_DEFAULT_PROVIDER "jd"
  #elif defined(ENABLE_PROVIDER_CUBE) && (ENABLE_PROVIDER_CUBE == 1)
    #define WUKONG_DEFAULT_PROVIDER "cube"
  #elif defined(ENABLE_PROVIDER_CLAW) && (ENABLE_PROVIDER_CLAW == 1)
    #define WUKONG_DEFAULT_PROVIDER "claw"
  #else
    #define WUKONG_DEFAULT_PROVIDER "tuya"
  #endif
#endif

#if defined(ENABLE_PROVIDER_CLAW) && (ENABLE_PROVIDER_CLAW == 1)
#include "wukong_agent_loop.h"   /* 头文件在 provider/claw/agent,非 claw 构建无此路径 */
#endif
...
STATIC VOID_T __agent_loop_thread(VOID_T *arg)
{
    ...
    while (TRUE) {
        ...
        if (prov->ops->llm_infer != NULL) {
            wukong_agent_process((CONST CHAR_T *)msg->chat_id, (CONST CHAR_T *)msg->channel,
                                 (CONST CHAR_T *)msg->data, prov);
        } else
#endif
        if (prov->ops->send) {
            if (prov->ops->ioctl) {
                BOOL_T interrupt = TRUE;
                prov->ops->ioctl(prov, WUKONG_PROVIDER_CMD_INPUT_START, &interrupt);
            } 
            prov->ops->send(prov, msg);
            if (prov->ops->ioctl) {
                prov->ops->ioctl(prov, WUKONG_PROVIDER_CMD_INPUT_STOP, NULL);
            }
        }
...
#if defined(ENABLE_PROVIDER_CLAW) && (ENABLE_PROVIDER_CLAW == 1)
        /* Claw 在本线程同步做 HTTP+TLS,握手耗栈,需加大 */
        .stackDepth = 16384,
#else
        .stackDepth = 4096,
#endif
...
#if defined(ENABLE_PROVIDER_JD) && (ENABLE_PROVIDER_JD == 1)
    extern WUKONG_AI_PROVIDER_T *wukong_provider_jd_get(VOID_T);
    wukong_ai_provider_register(wukong_provider_jd_get());
#endif

#if defined(ENABLE_PROVIDER_CUBE) && (ENABLE_PROVIDER_CUBE == 1)
    extern WUKONG_AI_PROVIDER_T *wukong_provider_cube_get(VOID_T);
    wukong_ai_provider_register(wukong_provider_cube_get());
#endif

#if defined(ENABLE_PROVIDER_CLAW) && (ENABLE_PROVIDER_CLAW == 1)
    extern WUKONG_AI_PROVIDER_T *wukong_provider_claw_get(VOID_T);
    wukong_ai_provider_register(wukong_provider_claw_get());
#endif
...
OPERATE_RET wukong_ai_agent_del_session(CONST CHAR_T *scode)
{
    if (s_prov_mgr.cur_provider == NULL || s_prov_mgr.cur_provider->ops == NULL) return OPRT_COM_ERROR;
    if (s_prov_mgr.cur_provider->ops->ioctl == NULL) return OPRT_NOT_SUPPORTED;
    return s_prov_mgr.cur_provider->ops->ioctl(s_prov_mgr.cur_provider,
                                                WUKONG_PROVIDER_CMD_DEL_SESSION, (VOID_T *)scode);
}

OPERATE_RET wukong_ai_agent_mcp_send(CHAR_T *sid, CHAR_T *eid, CONST CHAR_T *message)
{
    /* Polymorphic outbound: whichever cloud is active answers. Providers without an
     * MCP channel leave ops->mcp_send NULL and the call degrades gracefully. */
    WUKONG_AI_PROVIDER_T *p = s_prov_mgr.cur_provider;
    if (p == NULL || p->ops == NULL || p->ops->mcp_send == NULL)
        return OPRT_COM_ERROR;
    /* Pass the provider pointer (not p->ctx): ops methods receive the provider
     * handle and extract ctx themselves (e.g. cube's __cube_ctx(handle)). */
    return p->ops->mcp_send(p, sid, eid, message);
}
...
{
    if (type == WUKONG_AI_EVENT_TEXT_STREAM_START ||
        type == WUKONG_AI_EVENT_TEXT_STREAM_DATA ||
        type == WUKONG_AI_EVENT_TEXT_STREAM_STOP ||
        type == WUKONG_AI_EVENT_TEXT_STREAM_ABORT) {
        WUKONG_AI_TEXT_T *text = (WUKONG_AI_TEXT_T *)data;
        CONST CHAR_T *chunk = (text != NULL) ? text->data : NULL;
        UINT_T chunk_len = (text != NULL) ? text->datalen : 0U;
        if (wukong_ai_channel_dispatch_reply(type, chunk, chunk_len)) {
            return;  /* IM channel: do NOT forward to board event_cb */
        }
    }
