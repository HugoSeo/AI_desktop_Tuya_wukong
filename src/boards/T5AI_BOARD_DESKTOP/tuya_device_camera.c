...
/* External helpers provided by the DESKTOP UI layer (desk_func_camera.c).
 * They are invoked from the frame callbacks while a display stream is active:
 *   - camera_to_rgb565: YUV → RGB565 conversion for LVGL canvas (DVP preview)
 *   - camera_to_jpg   : save an MJPEG frame to the take-photo file on flash */
...
/* ---------------------------------------------------------------------------
 * Raw frame dispatch: TAL_CAMERA_FRAME_T → CAM_FRAME_T → adapter raw_cb.
 * --------------------------------------------------------------------------- */
...
{
    /* DESKTOP-specific LVGL canvas preview render (DVP only). */
...
{
    /* DESKTOP-specific: persist take-photo frame to flash while MJPEG runs. */
