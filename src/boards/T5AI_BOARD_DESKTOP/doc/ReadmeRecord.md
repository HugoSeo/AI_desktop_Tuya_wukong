...
| `src/ui/pages/ui_page_recording.c`、`src/ui/pages/ui_page_recording_list.c`、`src/ui/services/ui_svc_recording.c` | 录音 UI、文件管理、播放与上传（替代旧版 `ui/desk_func_record.h/.c`，已于 2026-06-25 随旧 UI 退役） |
...
用户进入录音屏
    ...
    └─→ register_audio_cb()（当前实现，`src/ui/services/ui_svc_recording.c`；旧版为 desk_record_handle_register()）
         └─ 注册 input_audio_cb 为 AI_RECORD_HANDLE_T.input_audio（经 wukong_ai_record_handle_set()）
    │
    │ [用户长按按键或 UI 点击录音按钮]
    │
    ├─→ 状态 → LISTEN
