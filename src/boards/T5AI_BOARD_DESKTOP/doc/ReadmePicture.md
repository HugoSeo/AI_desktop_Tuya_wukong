...
本节 11.1-11.7 描述的是 `T5AI_BOARD_DESKTOP` 板型下旧版 UI（src/boards/T5AI_BOARD_DESKTOP/ui/ 目录，已于 2026-06-25 随旧 UI 一并退役删除）中，生图模式从状态机事件到屏幕渲染的完整 UI 链路，供历史参考。当前实现：显示消息总线为 `src/miscs/display/tuya_ai_display.c`（不含 LVGL/GUI 代码，仅提供 `tuya_ai_display_msg()` + 回调注册的 UI 无关消息总线，不再有链表/mutex/LVGL 锁逻辑），默认悟空 UI 的回调实现见 `src/ui/port/tuya_ai_display_stub.c`（`ui_display_msg_handler()`），聊天页面渲染见 `src/ui/pages/ui_page_chat.c`。
