...
| `src/ui/pages/ui_page_detection.c` | 侦测记录 UI 屏幕实现（替代旧版 `ui/desk_func_detection.c/h`，已于 2026-06-25 随旧 UI 退役） |
...
侦测记录屏幕（当前实现 `src/ui/pages/ui_page_detection.c`；旧版 `desk_func_detection.c` 已于 2026-06-25 随旧 UI 退役）提供历史侦测事件的分页浏览能力。
...
云端响应 → TEXT_STREAM → 在 CHAT 屏聊天列表中流式显示分析结果气泡
    │
    ├─ 若当前在首页：ui_route_push(UI_PAGE_CHAT)（当前实现，`src/ui/port/tuya_ai_display_stub.c`；旧版为 desk_handle_ui_switch_to(DHUI_SCREEN_ID_CHAT)）
