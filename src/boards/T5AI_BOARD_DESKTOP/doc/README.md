...
UI 采用统一的页面导航框架，当前实现为 `ui_route`（`src/ui/core/ui_route.c` / `src/ui/core/ui_route.h`）；以下页面 ID、切换模式与注册结构描述的是 T5AI_BOARD_DESKTOP 旧版 UI（`desk_handle_ui`，已于 2026-06-25 随旧 UI 一并退役）的历史实现，作为设计参考，当时共定义 **19 个页面**：
...
- **顶部**：AI 头像图标（`ai_icon.png`）+ 当前模式标签（通过聊天页面内部的 `refresh_mode_label()` 刷新，`src/ui/pages/ui_page_chat.c`）
...
AI 回复文本以流式方式逐步显示在消息气泡中，当前实现通过 `src/ui/pages/ui_page_chat.c` 内部的静态状态变量管理（如 `s_stream_label` 当前接收流式文本的 LVGL 标签对象、`s_stream_pending_layout` 待布局刷新的字节计数），不再是独立的会话状态结构体。

旧版待处理通知消息机制（`desk_chat_set_pending_notify()` / `desk_chat_flush_pending_notify()`）已随旧 UI 退役，当前统一 UI 框架未提供对应能力。
...
- **角色名称**（`role_name`）：显示当前设备模式信息（如"闲聊模式: 按键"）；该"个人中心"聚合页本身随旧 UI（`desk_personal.c/h`）一并退役，当前统一 UI 框架（`src/ui/pages/`）无直接对应页面，各功能改由默认悟空页面集分别入口
- **功能网格**：8 个功能入口图标
...
- 切换后立即生效，设置持久化存储（当前实现存于统一 KV 键 `ui_settings` 的 JSON 字段 `language` 中，`src/ui/services/ui_svc_dev_ctrl.c`；旧版独立键名 `ai_desk_language` 已随旧 UI 退役）
...
| 18px | `AlibabaPuHuiTi3_Regular18` | 常规文字（静态编译） |
...
> 以下 UI 框架/页面/资源文件均为旧版 UI（`ui/desk_*`，已于 2026-06-25 随旧 UI 一并退役）实现，现已替换为统一 UI 框架 `src/ui/`；表中标注了各自的当前对应实现，无对应者已注明。

### UI 框架文件

| 文件 | 说明 |
|------|------|
| `src/ui/ui_app.c` | UI 应用入口：`ui_app_init()` 初始化，消息经 `src/ui/port/tuya_ai_display_stub.c` 的 `ui_display_msg_handler()` 分发（替代旧版 `ui/desktop_app.c`） |
| `src/ui/core/ui_route.c` / `src/ui/core/ui_route.h` | 页面导航框架：页面注册、路由切换（push/pop/replace/reset）、返回栈管理（替代旧版 `ui/desk_handle_ui.c/h`） |
| `src/ui/core/ui_state.c` / `src/ui/core/ui_state.h`、`src/ui/core/ui_tick.c` / `src/ui/core/ui_tick.h` | 单向数据流状态分发：业务线程写 `ui_state_set_*()` 置脏位，`ui_tick` 消费刷新页面（替代旧版 `ui/desk_event_handle.c/h` 的全局状态管理部分；手势/按键改由各页面自行处理，见 `src/ui/port/ui_port_indev.c`） |

### UI 页面文件

| 文件 | 说明 |
|------|------|
| （无对应文件） | 启动引导：启动画面 → 语言选择 → 二维码配网 → 网络配置；该 DESKTOP 专属启动流程随旧 UI（`ui/desk_startup.c/h`）一并退役，当前统一 UI 框架未提供对应页面 |
| `src/ui/pages/ui_page_home.c` | 首页：替代旧版 `ui/desk_home.c/h`（表情 GIF 轮播、时钟日期、快捷设置滑块） |
| `src/ui/pages/ui_page_chat.c` | 聊天页面：消息气泡、流式文本、图片展示、模式标签（替代旧版 `ui/desk_chat.c/h`） |
| （无对应文件） | 个人中心：功能网格、角色名称、设备模式选择页；该聚合页随旧 UI（`ui/desk_personal.c/h`）一并退役，当前统一 UI 框架各功能改由默认悟空页面集（`src/ui/pages/`）分别入口 |
| `src/ui/pages/ui_page_music.c`、`src/ui/pages/ui_page_music_list.c` | 音乐播放器：主播放页 + 播放列表页（替代旧版 `ui/desk_func_music.c/h`） |
| `src/ui/pages/ui_page_camera.c` | 相机：实时预览、AI 相机开关、拍照（替代旧版 `ui/desk_func_camera.c/h`） |
| `src/ui/pages/ui_page_photo.c` | 相册：照片浏览、删除、AI 分析上传（替代旧版 `ui/desk_func_photo.c/h`） |
| `src/ui/pages/ui_page_recording.c`、`src/ui/pages/ui_page_recording_list.c` | 录音：录制、暂停、保存 + 录音列表播放管理（替代旧版 `ui/desk_func_record.c/h`） |
| `src/ui/pages/ui_page_call.c` | P2P 通话：呼叫/挂断、状态显示（替代旧版 `ui/desk_func_call.c/h`） |
| `src/ui/pages/ui_page_detection.c` | 侦测：告警消息列表、分页、手动触发（替代旧版 `ui/desk_func_detection.c/h`） |
| `src/ui/pages/ui_page_settings.c` | 设置：网络配置、语言切换（替代旧版 `ui/desk_func_settings.c/h`） |

### 资源文件

| 文件 | 说明 |
|------|------|
| `src/ui/assets/icon/`、`src/ui/assets/font/` | 图标（各图标独立 `.c`/`.png` 文件）与预编译字体数据（阿里巴巴普惠体 3，多种字号）；旧版汇总资源头文件 `ui/res/desk_ui_res.h` 已随旧 UI 退役，无单一汇总头文件替代 |
