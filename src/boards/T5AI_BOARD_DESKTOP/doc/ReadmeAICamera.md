...
| 文件 | 说明 |
|------|------|
| `src/miscs/camera/tuya_camera.h/.c` | 摄像头服务（预览/拍照/侦测帧接口；替代旧版板级 `tuya_device_camera.h/.c`） |
| `src/ui/pages/ui_page_camera.c` + `src/ui/services/ui_svc_camera.c` / `src/ui/services/ui_svc_camera.h` | 相机预览屏 UI 与拍照逻辑（替代旧版 `ui/desk_func_camera.h/.c`，已于 2026-06-25 随旧 UI 退役） |
| `src/ui/pages/ui_page_photo.c` | 本地相册 UI 与文件管理（替代旧版 `ui/desk_func_photo.h/.c`，已于 2026-06-25 随旧 UI 退役） |
...
| `src/miscs/image_album/include/image_album.h` | 通用相册引擎（保存/读取/删除/迭代） |
| `src/miscs/image_album/include/image_album_thumb.h` | 缩略图生成 |
...
## 5. 相机预览屏（历史实现 `desk_func_camera.c`，已于 2026-06-25 随旧 UI 退役）

> 以下 5.1-5.3 节描述的是旧版 UI（`desk_func_camera.c`）实现细节，供历史参考。当前实现见 `src/ui/pages/ui_page_camera.c`（页面/交互）与 `src/ui/services/ui_svc_camera.c`（相机数据服务），不再使用 `CAMERA_DISPLAY_T` 结构体或 `desk_camera_*` 函数。
...
## 6. 本地相册（历史实现 `desk_func_photo.c`，已于 2026-06-25 随旧 UI 退役）

> 以下 6.1-6.2 节描述的是旧版 UI（`desk_func_photo.c`）实现细节，供历史参考。当前实现见 `src/ui/pages/ui_page_photo.c`，不再使用 `desk_photo_*` 函数。
