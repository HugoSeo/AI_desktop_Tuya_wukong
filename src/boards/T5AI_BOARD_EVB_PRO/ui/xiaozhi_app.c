...
VOID SetChatMessage(CONST UINT8_T* role, CONST CHAR_T* content) {
    ...
    /* AI 内容可能为 NULL;lv_label_set_text 对 NULL 文本会 strlen(NULL) 崩。 */
    if (content == NULL) {
        content = "";
    }
        ...
        {   
            ...
            if (buf_len < MAX_TEXT_LEN_IN_ONE_LABEL) {
                if (tmp_len < MAX_TEXT_LEN_IN_ONE_LABEL - buf_len) {
                    ...
                    // data 是 (data,len) 长度定界、不保证 '\0' 结尾,不能用 strlen 读裸指针:
                    // 改按 tmp_len 追加(strncat 至多 tmp_len 字节并自动补 '\0')。
                    strncat(buf, (CONST CHAR_T *)tmp_data, tmp_len);
                    buf_len += tmp_len;
