...
#define GUI_LOG(...)        TAL_PR_DEBUG(__VA_ARGS__)   /* was bk_printf (old gui); use tal_log under new framework */
...
/* 状态栏动图已迁 TEF 并下沉到消费方（ROBOT 板自维护 stat->TEF 表），
 * 公共层只管文本；res/status/ 的 GIF 资产随迁移删除。 */
...
INT_T gui_status_desc_get(UINT8_T stat,  VOID **text)
{
    INT_T i = 0;
...
{
    /* text 可能为 NULL:下面的 "%s" 日志、lv_label_set_text/ins_text 都会 strlen(NULL) 崩。 */
    if (text == NULL) {
        text = "";
    }
