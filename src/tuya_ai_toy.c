...
#endif
#include "tal_sleep.h"
#if defined(TY_AI_LOW_POWER_MODE) && (TY_AI_LOW_POWER_MODE == 2)
#include "lpmgr.h"
#endif
...
#endif
#if defined(ENABLE_CHAN_WECHAT) && (ENABLE_CHAN_WECHAT == 1)
#include "im_wechat_channel.h"
#endif
#if defined(ENABLE_CHAN_FEISHU) && (ENABLE_CHAN_FEISHU == 1)
#include "feishu_channel.h"
#endif
#if (defined(ENABLE_TUYA_CAMERA) && (ENABLE_TUYA_CAMERA == 1)) || \
    (defined(ENABLE_AI_MODE_P2P) && (ENABLE_AI_MODE_P2P == 1))
...
STATIC OPERATE_RET __on_ai_toy_reset(VOID *data)
{
    ...
    /* Deinit runtime state and clear persisted KV data so expired
     * once-alarms and old reminders do not survive across resets. */
    TUYA_CALL_ERR_LOG(wukong_time_manage_deinit());
    TUYA_CALL_ERR_LOG(wukong_time_manage_kv_clear());
...
/* ---------------------------------------------------------------------------
 * IM channel callbacks
 * --------------------------------------------------------------------------- */

#if defined(ENABLE_CHAN_WECHAT) && (ENABLE_CHAN_WECHAT == 1)
/**
 * @brief Called by the WeChat channel when a QR login URL is ready.
 * @param[in] qr_url WeChat QR login URL string.
 * @return none
 */
STATIC VOID_T __on_wechat_qr_notify(CONST CHAR_T *qr_url)
{
    TAL_PR_NOTICE("WeChat QR login URL: %s", qr_url ? qr_url : "(null)");
}
#endif

/**
 * @brief One-time handler for MQTT connected: start all IM channels.
 * @param[in] data unused
 * @return OPRT_OK
 */
STATIC OPERATE_RET __on_ai_toy_chan_mqtt_connected(VOID_T *data)
{
    (VOID_T)data;
    TAL_PR_NOTICE("MQTT connected, starting IM channels");
...
STATIC OPERATE_RET __ai_toy_wukong_ai_agent_init(VOID)
{
    ...
    /* Register IM channels, then call init on all registered channels. */
#if defined(ENABLE_CHAN_WECHAT) && (ENABLE_CHAN_WECHAT == 1)
    STATIC CONST WUKONG_CHAN_WECHAT_CFG_T s_wechat_cfg = {
        .qr_notify_cb = __on_wechat_qr_notify,
    };
    TUYA_CALL_ERR_LOG(im_wechat_channel_register(&s_wechat_cfg));
#endif
#if defined(ENABLE_CHAN_FEISHU) && (ENABLE_CHAN_FEISHU == 1)
    TUYA_CALL_ERR_LOG(feishu_channel_register());
#endif
    wukong_ai_channel_init_all();
    ...
    audio_cfg.board.sample_rate    = 16000;
    audio_cfg.board.sample_bits    = 16;
    audio_cfg.board.channel        = 1;
    ...
    // TUYA_CALL_ERR_LOG(wukong_kws_default_init());    //Deleted by Hugo
    return rt;
}
...
STATIC OPERATE_RET __ai_toy_stop(VOID)
{
    ...
    /* Only the mcp server is torn down here — it is the sole start/stop-scoped
     * object. The tool registry (wukong_tool s_tools) that tools/fc/skill add
     * to is a process-wide shared table (claw provider registers into it too),
     * so deinit must NOT drop it; wukong_toolkits_init's re-adds are idempotent
     * (register de-dups by name) and skill has its own lifecycle guard. */
    TUYA_CALL_ERR_LOG(wukong_mcp_deinit());
#endif
...
STATIC OPERATE_RET __ai_toy_start(VOID)
{
    ...
    ty_subscribe_event(EVENT_MQTT_CONNECTED,     "ai_toy", __on_ai_toy_chan_mqtt_connected, SUBSCRIBE_TYPE_ONETIME);
...
#endif
#if defined(ENABLE_TUYA_TOOLKITS) && (ENABLE_TUYA_TOOLKITS == 1)
    TUYA_CALL_ERR_GOTO(wukong_toolkits_init(), __error);
#endif
...
STATIC VOID __on_ai_toy_key_press_exit_lowpower()
{
    ...
    if (s_ai_toy->lp_stat) {      
        ...
        /* Exit low-power: ultra re-locks APP_USED (lpmgr); light/deep disable CPU/WiFi LP. */
#if (TY_AI_LOW_POWER_MODE == 2)
...
#if defined(ENABLE_TUYA_UI) && (ENABLE_TUYA_UI == 1)
    /** Close LCD backlight */
    tuya_display_hw_backlight_close();
#endif
...
#if (TY_AI_LOW_POWER_MODE == 2)
    else if (TY_AI_LOW_POWER_MODE == 2) {
        /* Ultra sleep (AP keep-alive): release app-used lock, let lpmgr take the
         * system into WiFi DTIM-10 sleep. AP stays alive, no reboot on wake. */
        lpmgr_unregister(TY_LP_APP_USED);
        s_ai_toy->lp_stat = TRUE;
        TAL_PR_DEBUG("ultra: lpmgr_unregister APP_USED");
    }
#endif
    else {
...
STATIC VOID __on_ai_toy_net_pin(UINT_T port, PUSH_KEY_TYPE_E type, INT_T cnt)
{
    ...
    case NORMAL_KEY:
        ...
        if (s_ai_toy && s_ai_toy->volume < 100) {
            ...
            if (s_ai_toy->volume % TOY_VOLUME_SETUP) {
                vol = (s_ai_toy->volume / TOY_VOLUME_SETUP + 1) * TOY_VOLUME_SETUP;
            } else {
                vol = s_ai_toy->volume + TOY_VOLUME_SETUP;
            }
            TAL_PR_DEBUG("volume %d", vol);
            tuya_ai_toy_volume_set(vol);
        }
        break;
    ...
    case SEQ_KEY:
        ...
        if (s_ai_toy && s_ai_toy->volume > 0) {
            UINT8_T vol;
            if (s_ai_toy->volume % TOY_VOLUME_SETUP) {
                vol = (s_ai_toy->volume / TOY_VOLUME_SETUP) * TOY_VOLUME_SETUP;
            } else {
                vol = s_ai_toy->volume - TOY_VOLUME_SETUP;
            }
            TAL_PR_DEBUG("volume %d", vol);
...
{
    TAL_PR_DEBUG("ai toy -> audio trigger pin pressed %d, mem left:%d", type, tal_system_get_free_heap_size());
...
#if (defined(ENABLE_TUYA_CAMERA) && (ENABLE_TUYA_CAMERA == 1)) || \
    (defined(ENABLE_AI_MODE_P2P) && (ENABLE_AI_MODE_P2P == 1))
    TAL_PR_NOTICE("ai toy -> init video service");
    wukong_video_init();
#endif
#if defined(ENABLE_TUYA_CAMERA) && (ENABLE_TUYA_CAMERA == 1)
...
#if defined(ENABLE_CLI_TEXT_INPUT) && (ENABLE_CLI_TEXT_INPUT == 1)
    extern int wukong_cli_text_init(void);
    wukong_cli_text_init();
    TAL_PR_NOTICE("ai toy -> cli text input enabled");
#endif
    ...
    for (UINT_T index = 0; index < dp->dps_cnt; index++) {
        ...
        if (dp->dps[index].dpid == 8) {
            ...
            if (dp->dps[index].value.dp_value <= 100 && dp->dps[index].value.dp_value >= 0 && s_ai_toy->volume != dp->dps[index].value.dp_value) {
                tuya_ai_toy_volume_set((UINT8_T)dp->dps[index].value.dp_value);
            }
        }
    }
}
...
BOOL_T tuya_ai_toy_is_cloud_connected(VOID)
{
#if defined(ENABLE_WIFI_SERVICE) && (ENABLE_WIFI_SERVICE == 1)
    return (s_ai_toy != NULL && s_ai_toy->nw_stat == STAT_CLOUD_CONN) ? TRUE : FALSE;
#else
    return FALSE;
#endif
}
...
OPERATE_RET tuya_ai_toy_volume_set(UINT8_T value)
{
    ...
    /* KV write blocks on flash; keep it behind the audible/visible response. */
