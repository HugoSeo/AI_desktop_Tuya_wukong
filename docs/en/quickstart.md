<!-- Auto-translated from ../quickstart.md. Do not edit manually. -->
...
> Display boards (`CONFIG_ENABLE_TUYA_UI=y`, e.g. the default T5AI_BOARD firmware) boot into an **on-screen activation wizard** on first power-up: after choosing a language, pick one of two paths — "App activation" returns to the home page and waits for the app to provision as described below; "Manual activation" connects to Wi‑Fi on screen and then shows a QR code that you scan with the Tuya Smart app to activate directly, no app-side device discovery needed. The wizard can be skipped at any time without affecting app provisioning.

1. Open the Tuya Smart app and start adding a device.
2. Choose the device type (e.g. Wi‑Fi device) and follow the app to put the device in provisioning mode (e.g. hold the provisioning button; on boards without one, power-cycle 3 times within 5 seconds — see [Pairing/Activation Troubleshooting](troubleshooting/network-pairing.md)).
