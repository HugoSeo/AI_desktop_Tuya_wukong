...
> 带屏板型（`CONFIG_ENABLE_TUYA_UI=y`，如 T5AI_BOARD 默认固件）首次开机会自动进入**屏上激活向导**：选择语言后可二选一——「APP 激活」回主页等待 App 按下述流程配网；「手动激活」在屏上连接 Wi-Fi 后展示二维码，用涂鸦智能 App 扫码即可直连激活，无需 App 搜索设备。向导可随时「跳过」，不影响 App 配网。

1. 打开涂鸦智能 App，进入添加设备流程。
2. 根据设备类型选择「Wi-Fi 设备」或对应品类，按 App 提示使设备进入配网模式（如长按配网键等；无配网键的板型可 5 秒内连续上电 3 次，详见[配网/激活失败排查](troubleshooting/network-pairing.md)）。
